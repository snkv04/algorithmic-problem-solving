...
pair<ll, ll> eval(int &idx, const string &s) {
    if (matrices.find(s[idx]) != matrices.end()) {
        return matrices[s[idx++]];
    } else if (s[idx] == '(' || s[idx] == ')') {
        ++idx;
        pair<ll, ll> left = eval(idx, s);
        if (left.first == -1) {
            return error;
        }
        pair<ll, ll> right = eval(idx, s);
        if (right.first == -1) {
            return error;
        }
        ...
        if (left.second == right.first) {
            ...
            pair<ll, ll> result = make_pair(left.first, right.second);
            ++idx;
            return result;
        } else {
            return error;
...
void solve() {
    ...
    while (getline(cin, line)) {
        ans = 0;
        int idx = 0;
        pair<ll, ll> result = eval(idx, line);
        if (result.first == -1) {
            cout << "error\n";
        } else {
            cout << ans << '\n';
