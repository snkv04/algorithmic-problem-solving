...
void solve() {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        for (int j = 0; j < tokens.size(); j += 2) {
            bool on = tokens[j][0] != '~';
            int var = on
                ? stoi(tokens[j].substr(1, tokens[j].size() - 1)) - 1
                : stoi(tokens[j].substr(2, tokens[j].size() - 2)) - 1;
            clauses[i].push_back(make_pair(var, on));
        }
    }
    ...
    for (int mask = 0; mask < (1 << n); ++mask) {
        ...
        for (int i = 0; i < m; ++i) {
            bool one = false;
            for (auto clause : clauses[i]) {
                bool val = (mask & (1 << clause.first)) > 0;
                if (val == clause.second) {
                    one = true;
                    break;
                }
            }
