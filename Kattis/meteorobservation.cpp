...
void solve() {
    int n;
    cin >> n;
    vector<pair<ll, ll>> meteors(n);
    for (int i = 0; i < n; ++i) {
        ll xi, yi;
        cin >> xi >> yi;
        meteors[i] = make_pair(xi, yi);
    }
    ...
    for (int i = 0; i < n; ++i) {
        ld angle = 180.0 * atan2((ld) meteors[i].second - yt, (ld) meteors[i].first - xt) / acos(-1.0);
        angles[2*i] = angle;
        angles[2*i+1] = angle + 360;
    }
    sort(angles.begin(), angles.end());
    // print_container(angles, "angles = ");
    // for (ld angle : angles) cout << angle << " "; // thought my eyes were deceiving me, but nah, the integers were because i set ld to long long :skull:
    // cout << endl;
    ...
    while (j < 2*n) {
        ++j;
        while (i < j && angles[j-1] - angles[i] > theta) {
            ++i;
        }
        ans = max(ans, j - i);
    }
