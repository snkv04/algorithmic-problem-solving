...
void solve() {
    ...
    - if we just want to extract some answer object from a situation where some quantity is optimized, then
    we can do binary search without actually saving the "last true" (or equivalent) value for that quantity,
    but instead just saving the answer object every time the quantity is valid (because then the last-saved
    answer will be from when the quantity was optimized).
    */
    ...
    while (l <= r) {
        ...
        vector<vector<bool>> possible(n+1, vector<bool>(height + 1, false));
        possible[0][0] = true;
        for (int i = 1; i <= n; ++i) {
            for (int j = 0; j <= height; ++j) {
                if (j - a[i-1] >= 0 && possible[i-1][j-a[i-1]]) {
                    possible[i][j] = true;
                }
                if (j + a[i-1] <= height && possible[i-1][j+a[i-1]]) {
                    possible[i][j] = true;
                }
            }
        }
        ...
        if (possible[n][0]) {
            ...
            int curr = 0;
            for (int i = n; i >= 1; --i) {
                if (curr + a[i-1] <= height && possible[i-1][curr + a[i-1]]) {
                    path.push('D');
                    curr += a[i-1];
                } else {
                    path.push('U');
