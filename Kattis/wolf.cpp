...
void solve() {
    ...
    set<pair<int, char>> ours, others;
    for (int i = 1; i <= 13; ++i) {
        for (char c : vector<char>({'C', 'D', 'H', 'S'})) {
            others.insert(make_pair(i, c));
        }
    }
    for (int i = 0; i < n; ++i) {
        int rank; char suit;
        cin >> rank >> suit;
        pair<int, char> p = {rank, suit};
        others.erase(p);
        ours.insert(p);
    }
    // cout << "ours.size() = " << ours.size() << "\n";
    // cout << "others.size() = " << others.size() << "\n";
