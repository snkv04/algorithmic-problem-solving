...
void solve() {
    int k;
    cin >> k;
    unordered_set<int> s; while (k--) { int si; cin >> si; s.insert(si); }

    vector<int> nimber(10001, 0);
    for (int i = 1; i <= 10000; ++i) {
        unordered_set<int> reachable;
        for (int move : s) {
            if (i >= move) reachable.insert(nimber[i-move]);
        }

        int mex = 0;
        while (reachable.find(mex) != reachable.end()) ++mex;
        nimber[i] = mex;
    }
