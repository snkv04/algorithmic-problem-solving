...
void dfs(int idx, string &curr, int end, int side, string &s, vector<map<string, ll>> &counts) {
    if (idx == end) {
        counts[side][curr] += 1;
        // cout << "reached with " << curr << endl;
        return;
    }
    ...
    if (s[idx] == '?') {
        for (auto e : toclose) {
            curr += e.first;
            dfs(idx+1, curr, end, side, s, counts);
            curr.pop_back();
        }

        for (auto e : toopen) {
            if (curr.size() && curr.back() == e.second) {
                char popping = curr.back();
                curr.pop_back();
                dfs(idx+1, curr, end, side, s, counts);
                curr.push_back(popping);
            }
        }
    } else {
        char c = s[idx];
        if (toclose.find(c) != toclose.end()) {
            curr += c;
            dfs(idx+1, curr, end, side, s, counts);
            curr.pop_back();
        } else if (curr.size() && curr.back() == toopen[c]) {
            char popping = curr.back();
            curr.pop_back();
            dfs(idx+1, curr, end, side, s, counts);
            curr.push_back(popping);
...
string flip(string s) {
    ...
    for (int i = 0; i < s.size(); ++i) {
        if (toclose.find(s[i]) != toclose.end()) {
            s[i] = toclose[s[i]];
        } else if (toopen.find(s[i]) != toopen.end()) {
            s[i] = toopen[s[i]];
        }
    }
    return s;
...
void solve() {
    ...
    vector<map<string, ll>> counts(2);
    for (int i = 0; i < 2; ++i) {
        string curr = "";
        dfs(0, curr, n/2, i, s, counts);
        s = flip(s);
    }

    ll ans = 0;
    for (auto e : counts[0]) {
        if (counts[1].find(e.first) != counts[1].end()) {
            ans += e.second * counts[1][e.first];
        }
    }
    cout << ans << endl;
}
