...
bool check(ld need, vector<pair<ld, int>> days) {
    priority_queue<pair<ld, int>, vector<pair<ld, int>>, Comp> food;
    for (int i = 0; i < days.size(); ++i) {
        food.push(days[i]);
        while (food.top().second < i) {
            food.pop();
        }
        ...
        while (stillneed) {
            ...
            pair<ld, int> piece = food.top();
            food.pop();
            if (piece.first >= stillneed) {
                piece.first -= stillneed;
                stillneed = 0;
                food.push(piece);
                break;
            } else {
                stillneed -= piece.first;
            }
        }
    }
    return true;
...
void solve() {
    ...
    cout << fixed << setprecision(12) << (ans < eps ? -1 : (ans / k)) << endl;
}
