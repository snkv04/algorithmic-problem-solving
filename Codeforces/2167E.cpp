...
void solve() {
    ...
    a.insert(-x); a.insert(2*x);

    vector<int> b; for (int num : a) b.push_back(num);
    n = b.size();

    int l = 1, r = x, maxdist = 0;
    while (l <= r) {
        int m = (l + r) >> 1;
        int achievable = 0;
        for (int i = 0; i < n - 1; ++i) {
            int left = max(0, b[i] + m), right = min(x, b[i+1] - m);
            achievable += max(0, right - left + 1);
        }
    ...
    set<int> used;
    for (int i = 0; i < n-1 && used.size() < k; ++i) {
            int left = max(0, b[i] + maxdist), right = min(x, b[i+1] - maxdist);
        for (int j = left; j <= right && used.size() < k; ++j) {
            used.insert(j);
        }
    }
    for (int coord : used) {
