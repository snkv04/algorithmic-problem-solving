...
int frombase26(string s) {
    int len = s.size();
    int before = 1, adding = 26;
    for (int i = 1; i < len; ++i) {
        before += adding;
        adding *= 26;
    }
    ...
    for (int i = len-1; i >= 0; --i) {
        int asnum = s[i] - 'A';
        result += asnum * multiplier;
        multiplier *= 26;
    }
    return result + before;
}

string tobase26(int num) {
    int checking = 26, pow = 1;
    while (num > checking) {
        num -= checking;
        checking *= 26;
        ++pow;
...
void solve() {
    ...
    int n = s.size();
    int idxC = -1;
    for (int i = 1; i < n; ++i) if (s[i] == 'C') { idxC = i; break; }
    if (s[0] == 'R' && idxC != -1 && is_num(s[1])) {
        int row = stoi(s.substr(1, idxC-1)), col = stoi(s.substr(idxC+1, n-(idxC+1)));
        // cout << "row = " << row << ", col = " << col << endl;

        cout << tobase26(col) << row << "\n";
    } else {
        int idxnum = -1;
        for (int i = 0; i < n; ++i) if (is_num(s[i])) {
            idxnum = i;
            break;
        }

        string col = s.substr(0, idxnum);
        int row = stoi(s.substr(idxnum, n-idxnum));
        // cout << "col = " << col << ", row = " << row << endl;

        cout << "R" << row << "C" << frombase26(col) << "\n";
    }
}
