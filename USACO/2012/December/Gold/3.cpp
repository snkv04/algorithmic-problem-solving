...
vector<priority_queue<int, vector<int>, Comp>> subtree;
...
void dfs(int node, int parent) {
    subtree[node].push(node);
    for (auto next : adj[node]) {
        int nextnode = next.first;
        if (nextnode == parent) continue;

        depth[nextnode] = depth[node] + next.second;
        dfs(nextnode, node);

        if (subtree[nextnode].size() > subtree[node].size()) {
            swap(subtree[nextnode], subtree[node]);
        }
        while (subtree[nextnode].size()) {
            int top = subtree[nextnode].top();
            subtree[nextnode].pop();
            subtree[node].push(top);
        }
    }

    while (subtree[node].size() && depth[subtree[node].top()] > depth[node] + l) {
        subtree[node].pop();
    }
...
void solve() {
    ...
    for (int i = 1; i < n; ++i) {
        ...
        cin >> p >> li;
        --p;
        adj[p].push_back(make_pair(i, li));
    }
