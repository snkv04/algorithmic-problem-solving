...
void solve() {
    ...
    priority_queue<array<ll, 3>> meetings;
    set<int> remaining;
    for (int i = 0; i < n-1; ++i) {
        ll speed = s[i+1] + s[i];
        ll time = (p[i+1] - p[i] + speed - 1) / speed;
        if (i % 2) {
            meetings.push({-2 * time, i, i+1});
        } else {
            meetings.push({-2 * time + 1, i, i+1});
        }
    ...
    while (remaining.size()) {
        ll time = meetings.top()[0];
        vector<array<ll, 3>> popping;
        while (meetings.size() && meetings.top()[0] == time) {
            popping.push_back(meetings.top());
            meetings.pop();
        }

        time = -time;
        for (auto arr : popping) {
            if (remaining.find(arr[1]) == remaining.end() || remaining.find(arr[2]) == remaining.end()) continue;

            ans[arr[1]] = ans[arr[2]] = time;
            remaining.erase(arr[1]);
            remaining.erase(arr[2]);

            if (remaining.upper_bound(arr[1]) != remaining.end() && remaining.upper_bound(arr[1]) != remaining.begin()) {
                auto it = remaining.upper_bound(arr[1]);
                int right = *it;
                --it;
                int left = *it;

                ll speed = s[left] + s[right];
                ll time = (p[right] - p[left] + speed - 1) / speed;
                if (left % 2) {
                    meetings.push({-2 * time, left, right});
                } else {
                    meetings.push({-2 * time + 1, left, right});
                }
            }
        }
    }
