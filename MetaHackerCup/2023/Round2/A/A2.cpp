...
set<pair<int, int>> getempty(vector<pair<int, int>> &comp) {
    ...
    for (const pair<int, int> &cell : comp) {
        for (int k = 0; k < 4; ++k) {
            ...
            if (newi >= 0 && newi < r && newj >= 0 && newj < c && grid[newi][newj] == '.') {
                empty_adj.insert(make_pair(newi, newj));
            }
        }
    }
    return empty_adj;
...
void solve() {
    ...
    for (auto &comp : comps) {
        set<pair<int, int>> empty = getempty(comp);
        if (empty.size() == 1) {
            auto cell = *empty.begin();
            count[cell.first][cell.second] += comp.size();
        }
    }
