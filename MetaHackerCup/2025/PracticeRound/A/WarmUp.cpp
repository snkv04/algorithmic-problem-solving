...
void solve() {
    ...
    vector<vector<pair<int, int>>> plates(n+1);
    for (int i = 0; i < n; ++i) {
        plates[a[i]].push_back(make_pair(i + 1, b[i]));
    }

    vector<pair<int, int>> ans;
    for (int i = 1; i <= n; ++i) {
        for (pair<int, int> p : plates[i]) {
            int idx = p.first, target = p.second;
            if (plates[target].empty()) {
                cout << "-1\n";
                return;
            }
            if (target != i) {
                ans.push_back(make_pair(idx, plates[target][0].first));
            }
        }
    }

    cout << ans.size() << "\n";
    for (pair<int, int> operation : ans) {
