...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (
            i == n - 1 ||
            (inc && a[i+1] != a[i] + 1) ||
            (!inc && distinct.size() == *distinct.rbegin())
        ) {
            int size = distinct.size();
            distinct.clear();
            int shift = 0;
            if (!inc) {
                for (int j = last + size; j > last; --j) {
                    if (a[j] == 1) {
                        shift = last + size + 1 - j;
                        break;
                    }
            ...
            last = i;
            perms.push_back(make_pair(size, shift));
        }
    }
    // print_container(perms, "perms = ");

    stack<pair<int, int>> ans;
    int curr = 0;
    for (int i = perms.size() - 1; i >= 0; --i) {
        int need = (perms[i].second + perms[i].first - (curr % perms[i].first)) % perms[i].first;
        for (int j = 0; j < need; ++j) {
            ans.push(make_pair(2, -1));
        }
        curr += need;

        ans.push(make_pair(1, perms[i].first));
    }
