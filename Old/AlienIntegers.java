...
public class AlienIntegers {
    ...
    static void solve() throws IOException {
        char[] _n = br.readLine().toCharArray();
        int len = _n.length;
        int[] n = new int[len]; for (int i = 0; i <= len-1; i++) n[i] = (int) (_n[i]-'0');
        TreeSet<Integer> unused = new TreeSet<>(); for (int i =0; i <= 9; i++) unused.add(i);
        for (int i= 0; i <= len-1; i++) unused.remove(n[i]);
        if (unused.size()==0) { pw.println("Impossible"); return; }

        int[] above = new int[len], lower = new int[len]; Arrays.fill(lower, 9);
        above[0] = n[0]; lower[0] = n[0];
        boolean aboveWorks = true, belowWorks = true;
        for (int i = 0; i <= len-1; i++) {
            if (aboveWorks && !unused.contains(above[i])) {
                if (unused.higher(above[i]) != null) {
                    above[i] = unused.higher(above[i]);
                } else {
                    aboveWorks = false;
                }
            }

            if (belowWorks && !unused.contains(lower[i])) {
                if (unused.lower(lower[i]) != null) {
                    lower[i] = unused.lower(lower[i]);
                } else {
                    belowWorks = false;
                }
            }
        ...
        if (belowWorks) {
            results.add(getNum(lower));
        }
        {
            int[] arr = new int[len-1];
            Arrays.fill(arr, unused.last());
            results.add(getNum(arr));
        }
        if (aboveWorks) {
            results.add(getNum(above));
        }
        {
            int[] arr = new int[len+1];
            Arrays.fill(arr, unused.first());
            results.add(getNum(arr));
        }

        long newN = getNum(n), minDist = Long.MAX_VALUE / 4;
        for (long res : results) minDist = Math.min(minDist, Math.abs(res-newN));
        for (long res : results) if (Math.abs(res-newN) == minDist) pw.print(res+" ");
    }

    static long getNum(int[] a) {
        int len = a.length;
        long result = 0, multiplier = 1;
        for (int i = 0; i <= len-1; i++) {
            result += a[len-1-i] * multiplier;
            multiplier *= 10;
        }
        return result;
    }
