...
public class MissingNumber {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) solve();
        // int cnt = 0; for (int i= 1; i <= 99999; i++) cnt += toArray(i).length; pw.println(cnt);
    ...
    static void solve() throws IOException {
        ...
        for (int lenFirst = 1; lenFirst <= 5; lenFirst++) {
            ...
            if (!valid) continue;
            int skipped = 0;
            HashSet<Integer> addendum = new HashSet<>();
            for (int i = 1; i <= nums.size()-1; i++) {
                if (nums.get(i) != nums.get(i-1)+1) {
                    skipped++;
                    addendum.add(nums.get(i)-1);
                }
            }
            if (skipped == 0) {
                ...
                for (int num : addendum) possible.add(num);
            }
        }
