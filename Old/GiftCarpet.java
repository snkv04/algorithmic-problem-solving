...
public class GiftCarpet {
    public static void main(String[] args) throws IOException {
        ...
        TreeSet<Integer> v=new TreeSet<>(), I=new TreeSet<>(), k=new TreeSet<>(), a=new TreeSet<>();
        for (int j = 0; j <= m-1; j++) {
            for (int i = 0; i <= n-1; i++) {
                if (grid[i][j]=='v') {
                    v.add(j);
        ...
        if (works && I.higher(next) != null) {
            next = I.higher(next);
        } else works = false;

        if (works && k.higher(next) != null)  next=k.higher(next);
        else works = false;

        if (works && a.higher(next) != null) next = a.higher(next);
        else works = false;
