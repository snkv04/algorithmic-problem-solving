...
public class PlatformPlacing {
    ...
    static void solve() throws IOException {
        ...
        int n = Integer.parseInt(asdf[0]), s = Integer.parseInt(asdf[1]), l = Integer.parseInt(asdf[2]);
        s *= 2; l *= 2;
        Long[] centers = new Long[n];
        for (int i = 0; i <= n-1; i++) centers[i] = (long) 2 * Integer.parseInt(br.readLine());
        Arrays.sort(centers);

        long[][] endpoints = new long[n][2];
        for (int i = 0; i <= n-1; i++) {
            endpoints[i][0] = centers[i] - s/2;
            endpoints[i][1] = centers[i]+s/2;
        }
        // for (int i = 0; i <= n-1; i++) pw.println(Arrays.toString(endpoints[i]));

        boolean works = true;
        long ans = 0;
        for (int i = 0; i <= n-1; i++) {
            long lastRight = i==0?-Integer.MAX_VALUE:endpoints[i-1][1];
            long nextLeft = i==n-1?Integer.MAX_VALUE:endpoints[i+1][0];
            if (nextLeft < endpoints[i][1]-0.0001) {
                works = false;
                break;
            } else {
                long min = Math.min(endpoints[i][0]-lastRight, nextLeft-endpoints[i][1]);
                long half = Math.min(min+s/2, l/2);
                ans += half * 2;
                endpoints[i][1] = centers[i]+half;
            }
        }
        pw.println(works?ans/2:-1);
    }
