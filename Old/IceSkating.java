...
public class IceSkating {
    public static void main(String[] args) throws IOException {
        ...
        // ArrayList<Integer>[] pointsPerX = new ArrayList[1001], pointsPerY = new ArrayList[1001];
        // for (int i = 0; i <= 1000; i++) { pointsPerX[i] = new ArrayList<>(); pointsPerY[i] = new ArrayList<>(); }
        // for (int i = 0; i <= n-1; i++) {
        ...
        // DSU dsu = new DSU(n);
        // for (int i = 0; i <= n-1; i++) {
        //     for (int next : pointsPerX[x[i]]) {
        //         if (next != i) dsu.union(i, next);
        //     }
        //     for (int next : pointsPerY[y[i]]) if (next != i) dsu.union(i, next);
        // }

        // HashSet<Integer> components = new HashSet();
        // for (int i = 0; i <= n-1; i++) components.add(dsu.find(i));
        // pw.println(components.size()-1);

        // this solution is more brute force
        int n = Integer.parseInt(br.readLine());
        int[] x = new int[n], y = new int[n];
        for (int i = 0; i <= n-1; i++) {
            String[] line = br.readLine().split(" ");
        ...
        for (int i = 0; i <= n-1; i++) {
            for (int j = i+1; j <= n-1; j++) {
                if (x[i]==x[j] || y[i]==y[j]) dsu.union(i, j);
            }
        }
