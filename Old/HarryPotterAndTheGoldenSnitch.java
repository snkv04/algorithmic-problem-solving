...
public class HarryPotterAndTheGoldenSnitch {
    ...
    static void solve() throws IOException {
        ...
        coords = new double[n+1][3];
        for (int i = 0; i <= n; i++) {
            coords[i] = Arrays.stream(br.readLine().split(" ")).mapToDouble(Double::parseDouble).toArray();
        }
        double vs;
        String[] line = br.readLine().split(" ");
        vp = Double.parseDouble(line[0]); vs = Double.parseDouble(line[1]);
        p = Arrays.stream(br.readLine().split(" ")).mapToDouble(Double::parseDouble).toArray();

        t = new double[n+1];
        t[0] = 0;
        for (int i= 1; i <= n; i++) {
            double dx = coords[i][0] - coords[i-1][0], dy = coords[i][1]-coords[i-1][1], dz = coords[i][2]-coords[i-1][2];
            double dist = Math.sqrt(dx*dx + dy*dy + dz*dz);
            t[i] = t[i-1] + (dist / vs);
        }
        // pw.println(Arrays.toString(t));

        double l = 0, r = Integer.MAX_VALUE, ans = -1;
        ...
        if (ans != -1 && ans <= t[n] + 0.0000000001) {
            pw.println("YES\n"+ans);
            pw.println(ansloc[0]+" "+ansloc[1]+" "+ansloc[2]);
        } else {
            pw.println("NO");
        }
    }

    static boolean check(double time) {
        int l = 0, r = n, index = -1;
        while (l <= r) {
            int mid = (l+r)/2;
            if (t[mid] <= time) {
        ...
        if (index == n) {
            for (int i =0; i <= 2; i++) loc[i] = coords[index][i];
        } else {
            double[] vector = {
                coords[index+1][0]-coords[index][0],
                coords[index+1][1]-coords[index][1],
                coords[index+1][2]-coords[index][2]
            };
            // double magnitude = 0; for (int i =0 ;i  <= 2; i++) magnitude += vector[i]*vector[i];
            // magnitude = Math.sqrt(magnitude);
            // for (int i = 0; i <= 2; i++) vector[i] /= magnitude;
            double remainingTime = time - t[index];
            for (int i = 0; i <= 2; i++) {
                loc[i] = coords[index][i] + vector[i] * (remainingTime/(t[index+1]-t[index]));
            }
        }

        double dist = 0; for (int i = 0; i <= 2; i++) dist += (loc[i]-p[i]) * (loc[i]-p[i]);
        dist = Math.sqrt(dist);
        if (dist <= vp * time) {
            ansloc = new double[3];
            for (int i = 0; i <= 2; i++) ansloc[i] = loc[i];
            return true;
        } else
            return false;
    }
