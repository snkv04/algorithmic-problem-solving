...
public class InsertZeroAndInvertPrefix {
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            for (int i = 1; i <= n-1; i++) {
                if (a[i]==a[i-1]) {
                    segments.add(segments.remove(segments.size()-1)+1);
                } else {
                    segments.add(1);
                }
            }
            ...
            for (int i = segs-1; i>=1; i-=2) {
                int zeros = segments.get(i);
                int ones = segments.get(i-1);
                for (int j = 1; j <= zeros+ones-1; j++) pw.print("0 ");
                pw.print(ones+" ");
            }
            if (segs%2==1) {
                for (int i = 1; i <= segments.get(0); i++) pw.print("0 ");
            }
            pw.println();
        }
