...
public class ProblemClassification {
    ...
    static void solve() throws IOException {
        ...
        HashMap<String, HashSet<String>> wordToCats = new HashMap<>();
        for (int i = 0; i < n; i++) {
            String[] str = br.readLine().split(" ");
            int words = Integer.parseInt(str[1]);
            for (int j = 2; j < 2+words; j++) {
                if (!wordToCats.containsKey(str[j])) {
                    wordToCats.put(str[j], new HashSet<>());
                }
                wordToCats.get(str[j]).add(str[0]);
            }
        }
        // pw.println(wordToCats);

        String line;
        Multiset categories = new Multiset();
        while ((line = br.readLine()) != null) {
            String[] str = line.split(" ");
            for (String word : str) {
                if (wordToCats.containsKey(word)) {
                    for (String category : wordToCats.get(word)) {
        ...
        for (Map.Entry<String, Integer> entry : categories.mset.entrySet()) {
        ...
        if (maxFrequency == 0) {
            recommended.clear();
            for (Map.Entry<String, HashSet<String>> entry : wordToCats.entrySet()) {
                for (String cat : entry.getValue()) {
                    recommended.add(cat);
                }
            }
        }
        for (String cat : recommended) {
            pw.println(cat);
        }
    }
    ...
    static class Multiset { // ordered multiset
        ...
        public void add(String x, int q) {
            // q for quantity
            if (mset.containsKey(x)) {
