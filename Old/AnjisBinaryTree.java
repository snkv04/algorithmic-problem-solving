...
public class AnjisBinaryTree {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            for (int i = 0; i <= n-1; i++) {
                if (tree[i][0]==-1 && tree[i][1]==-1) ans = Math.min(ans, memo[i]);
            }
            pw.println(ans);
        }
    ...
    static void dfs(int node) {
        if (tree[node][0] != -1) {
            memo[tree[node][0]] = memo[node];
            if (s[node]!='L')memo[tree[node][0]]++;
            dfs(tree[node][0]);
        }
        if (tree[node][1] != -1) {
            memo[tree[node][1]] = memo[node];
            if (s[node]!='R')memo[tree[node][1]]++;
            dfs(tree[node][1]);
