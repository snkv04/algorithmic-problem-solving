...
public class MoneyTrees {
    ...
    static void solve() throws IOException {
        ...
        int n = Integer.parseInt(asdf[0]), k = Integer.parseInt(asdf[1]);
        ...
        lefts.add(0);
        for (int i = 1; i <= n-1; i++) {
            if (h[i-1] % h[i] != 0) lefts.add(i);
        }
        lefts.add(n);

        int ans = 0;
        for (int i = 0; i <= lefts.size() - 2; i++) {
            int left = lefts.get(i), right = lefts.get(i+1)-1;
