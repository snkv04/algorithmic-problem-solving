...
public class Microcycle {
    ...
    static void solve() throws IOException {
        ...
        for (int i = 0; i <= m-1; i++) {
            ...
            edges[i] = new Edge(Integer.parseInt(asdf[0]) - 1, Integer.parseInt(asdf[1]) - 1, Integer.parseInt(asdf[2]));
        }
        Arrays.sort(edges, Comparator.comparingInt(o -> -o.w));

        // MAXIMUM spanning tree (forest)
        mst = new HashSet[n]; for (int i = 0; i <= n-1; i++) mst[i] = new HashSet<Integer>();
        DSU dsu = new DSU(n);
        for (int i = 0; i <= m-1; i++) {
            Edge e = edges[i];
        ...
        for (int i = m - 1; i >= 0; i--) {
            Edge e = edges[i];
            if (!mst[e.a].contains(e.b)) {
                start =e ;
                break;
            }
        }

        mst[start.b].remove(start.a);
    ...
    static void dfs(int node) {
        visited[node] = true;
        stack.add(node);
        if (node == start.a && !done) {
            pw.println(start.w + " " + stack.size());
            for (int num : stack) pw.print(num+1+" ");
            pw.println();
            done = true;
        }
        if (done) {
