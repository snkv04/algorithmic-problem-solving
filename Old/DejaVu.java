...
public class DejaVu {
    ...
    static void solve() throws IOException {
        ...
        ArrayList<Integer> meaningful = new ArrayList<Integer>();
        for (int i = 0; i <= q-1; i++) {
            if (meaningful.isEmpty()) meaningful.add(x[i]);
            else if (x[i] < meaningful.get(meaningful.size() - 1)) meaningful.add(x[i]);
        }
        ...
        for (int i = 0; i <= n-1; i++) {
            ...
            for (int j = 0; j <= meaningful.size() - 1; j++) {
                if (meaningful.get(j) <= leftmost) a[i] = a[i] | (1<<(meaningful.get(j)-1));
            }
        }

        for (int i = 0; i <= n-1; i++) { pw.print(a[i]+" "); }
        pw.println();
    }
