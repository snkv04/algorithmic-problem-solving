...
public class BitwiseBalancing {
    ...
    static void solve() throws IOException {
        long[] bcd = Arrays.stream(br.readLine().split(" ")).mapToLong(Long::parseLong).toArray();
        List<Integer> a = new ArrayList<Integer>();
        while (bcd[0]+bcd[1]+bcd[2] > 0) {
            long b = bcd[0]%2, c = bcd[1]%2, d = bcd[2]%2;
            if (b == c && c == d) {
                a.add(0);
            } else if (b == c) {
                a.add(1);
            ...
            for (int i = 0; i <= 2; i++) bcd[i] /= 2;
        }

        long ans = 0;
        for (int i = 0; i < a.size(); i++) {
            ans += (long)a.get(i) * ((long)1 << i);
        }
        pw.println(ans);
    }
