...
public class Hamburgers {
    public static void main(String[] args) throws IOException {
        ...
        int[] need = new int[3];
        for (char letter : s) if (letter=='B')need[0]++; else if (letter=='S')need[1]++; else need[2]++;
        ...
        while (l <= r) {
            long mid = (l+r)/2;
            // pw.println("checking mid="+mid);
            long[] currNeed = new long[3];
            for (int i = 0; i <= 2; i++) currNeed[i] = Math.max(0, mid*need[i]-n[i]);
            long totalPrice = 0; for (int i = 0; i <= 2; i++) totalPrice += currNeed[i] * p[i];
            if (totalPrice <= money) {
                ans = mid;
                l = mid+1;
            } else {
