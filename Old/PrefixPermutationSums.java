...
public class PrefixPermutationSums {
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            TreeSet<Long> nums = new TreeSet<>(); for (int i = 1; i <= n; i++) nums.add((long)i);
            long sum = -1;
            boolean works = true;
            for (Map.Entry<Long, Integer> entry : diffs.mset.entrySet()) {
                if (nums.contains(entry.getKey())) {
                    if (entry.getValue()==1) {
            ...
            // pw.println(nums);
            // pw.println(sum);
            if (nums.size() == 2 && nums.first()+nums.last()==sum) ;
            else works = false;
            pw.println(works?"YES":"NO");
        }
