...
public class LandEquality {
    ...
    static void solve() throws IOException {
        ...
        int r = Integer.parseInt(asdf[0]), c = Integer.parseInt(asdf[1]);
        ...
        if (cnt[0] >= 2) pw.println(0);
        else if (cnt[0] == 1) {
            if (r == 1) {
                int ans = Math.min(grid[0][0]==0?Integer.MAX_VALUE:grid[0][0], grid[0][c-1]==0?Integer.MAX_VALUE:grid[0][c-1]);
                pw.println(ans);
                return;
            }
            if (c == 1) {
                int ans = Math.min(grid[0][0]==0?Integer.MAX_VALUE:grid[0][0], grid[r-1][0]==0?Integer.MAX_VALUE:grid[r-1][0]);
                pw.println(ans);
                return;
            }

            if (cnt[1] >= 1) pw.println(1);
            else pw.println(2);
        } else {
            if (cnt[2] > 0) {
                if (cnt[2]%2==0) pw.println(0);
                else {
                    pw.println(((long)1)<<(cnt[2]/2));
                }
            } else {
                // pw.println((r*c)%2);
                pw.println(0);
