...
public class AsteriskMinorTemplate {
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            TreeSet<String> apairs = new TreeSet();
            for (int i = 1; i <= a.length-1; i++) apairs.add(String.valueOf(a[i-1]) + String.valueOf(a[i]));
            TreeSet<String> bpairs = new TreeSet();
            for (int i = 1; i <= b.length-1; i++) bpairs.add(String.valueOf(b[i-1]) + String.valueOf(b[i]));
            ...
            for (String pair : apairs) {
                if (bpairs.contains(pair)) {
                    ans = "*" + pair + "*";
                    break;
                }
            }
            pw.println(ans==""?("NO"):("YES\n"+ans));
        }
