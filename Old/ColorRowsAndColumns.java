...
public class ColorRowsAndColumns {
    ...
    static void solve() throws IOException {
        ...
        for (int i = 0; i <= n-1; i++) {
            rects[i] = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        }
        ...
        for (int i = 0; i <= n-1; i++) {
            for (int j = k; j >= 1; j--) {
                ...
                while (b > 0) {
                    ...
                    b--;
                    if (j - points >= 0) {
                        memo[j] = Math.min(memo[j], memo[j-points] + ops);
                    }
                    if (a > b) {
                        int temp = a;
                        a = b;
        ...
        pw.println(memo[k]==Integer.MAX_VALUE/2?-1:memo[k]);
    }
