        ...
        while (t-->0) {
            ...
            if (n >= 3) for (int i = 1; i <= n-2; i++) {
                if ((a[i] < a[i-1] && a[i] < a[i+1]) || (a[i] > a[i-1] && a[i] > a[i+1])) {
                    long curr = Math.min(Math.abs(a[i]-a[i-1]), Math.abs(a[i]-a[i+1]));
                    reduce = Math.max(reduce, curr*2);
                }
            }
            if (n >= 2) {
                reduce = Math.max(reduce, Math.abs(a[0]-a[1]));
                reduce = Math.max(reduce, Math.abs(a[n-1]-a[n-2]));
            }
            pw.println(ans-reduce);
        }
