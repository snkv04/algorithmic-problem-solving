...
public class IWannaBeTheGuy {
    public static void main(String[] args) throws IOException {
        ...
        int p = sc.nextInt();
        for (int i = 1; i <= p; i++) {levels.add(sc.nextInt());}
        p = sc.nextInt();
        for (int i = 1; i <= p; i++) {levels.add(sc.nextInt());}
        Collections.sort(levels);
        boolean works = true;
        for (int i = 1; i <= levels.size()-1; i++) if (levels.get(i)-levels.get(i-1) > 1) { works = false; break; }
        pw.println((works&&levels.size()>=n&&levels.get(0)==1&&levels.get(levels.size()-1)==n)
            ?"I become the guy.":"Oh, my keyboard!");
