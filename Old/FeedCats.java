...
public class FeedCats {
    ...
    static void solve() throws IOException {
        ...
        Multiset lefts = new Multiset();
        PriorityQueue<Pair> rights = new PriorityQueue<>(Comparator.comparingInt(o -> o.b));
        ...
        int segmentPointer = 0, ans = 0;
        for (int i = 1; i <= n; i++) {
            while (segmentPointer <= m-1 && segments[segmentPointer] [0]== i) {
                lefts.add(segments[segmentPointer][0]);
                rights.add(new Pair(segmentPointer, segments[segmentPointer][1]));
                segmentPointer++;
            }
            while (!rights.isEmpty() && rights.peek().b < i) {
                Pair indexRight = rights.poll();
                lefts.remove(segments[indexRight.a][0]);
            }

            int leftmost = Integer.MAX_VALUE / 2;
            if (!lefts.mset.isEmpty()) {
                leftmost = Math.min(leftmost, lefts.mset.firstKey());
            }
            memo[i] = Math.max(memo[i-1], rights.size() + (leftmost==Integer.MAX_VALUE/2 ? 0 : memo[leftmost-1]));
