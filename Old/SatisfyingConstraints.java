...
public class SatisfyingConstraints {
    ...
    static void solve() throws IOException {
        ...
        while (n-->0) {
            ...
            if (asdf[0].equals("1")) {
                max = Math.max(max, Integer.parseInt(asdf[1]));
            } else if (asdf[0].equals("2")) {
                min = Math.min(min, Integer.parseInt(asdf[1]));
            } else {
                not.add(Integer.parseInt(asdf[1]));
            }
        }

        if (min < max) pw.println(0);
        else {
            int subtract = 0;
            for (int num : not) {
                if (num >= max && num <= min) subtract++;
            }
            pw.println(min-max+1-subtract);
        }
    }
