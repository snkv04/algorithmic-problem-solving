...
public class DivideAndEqualize {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (tc-- > 0) {
            ...
            Multiset factors = new Multiset();
            for (int i = 0; i <= n-1; i++) {
                ArrayList<Integer> primes = primeFactorization(a[i]);
                // pw.println("a[i]="+a[i]+", primes="+primes);
                for (int prime : primes) factors.add(prime);
            }

            boolean works = true;
            for (Map.Entry<Integer, Integer> pair : factors.mset.entrySet()) {
                if (pair.getValue() % n != 0) {
                    works = false;
                    break;
                }
            }
