...
public class EqualRectangles {
    public static void main(String[] args) throws IOException {
        ...
        while (q-->0) {
            ...
            Integer[] b = new Integer[4*n];
            Multiset lengths = new Multiset();
            for (int i = 0; i <= 4*n-1; i++) {
                lengths.add(a[i]);
            }
            int area = lengths.mset.lastKey() * lengths.mset.firstKey();
            boolean works = true;
            while (lengths.mset.size() > 1) {
                int last = lengths.mset.lastKey();
                int first = lengths.mset.firstKey();
                if ((last * first == area) && (lengths.mset.get(first) %2==0)
                    && (lengths.mset.get(first).equals(lengths.mset.get(last)))) {
                    
                    lengths.mset.pollLastEntry();
                    lengths.mset.pollFirstEntry();
                } else {
                    works = false; break;
                }
            }
            if (lengths.mset.size()==1) {
                int length = lengths.mset.firstEntry().getKey();
                if (!(length*length==area && lengths.mset.firstEntry().getValue()%4==0)) {
                    works = false;
                }
            }
