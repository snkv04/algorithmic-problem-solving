...
public class DivisiblePairs {
    ...
    static void solve() throws IOException {
        ...
        int n = Integer.parseInt(asdf[0]), x = Integer.parseInt(asdf[1]), y = Integer.parseInt(asdf[2]);
        ...
        for (int num : a) {
            int modx = num % x;
            int mody = num % y;
            String lookingFor = ((x-modx)%x)+" "+mody, key = modx+" "+mody;
            if (counts.mset.containsKey(lookingFor)) {
                ans += counts.mset.get(lookingFor);
            }
            counts.add(key);
        }
        pw.println(ans);
