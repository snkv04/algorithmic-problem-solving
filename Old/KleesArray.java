...
public class KleesArray {
    ...
    // static long calc(long i, long n, long k) {
    //     long prefix = (i * (i + 1) / 2) + (k - 1) * i;
    //     long suffix = ((n-i) * (n-i+1) / 2) + (n-i) * (k-1+i);
    //     return Math.abs(prefix-suffix);
    // }

    static long prefix(long i, long n, long k) {
        return (i * (i + 1) / 2) + ((long)k - 1) * i;
    }

    static long suffix(long i, long n, long k) {
        return (long) (((n-i) * (n-i+1) / 2) + (n-i) * (k-1+i));
    }

    static void solve() throws IOException {
        // String[] s = br.readLine().split(" ");
        // double n = Integer.parseInt(s[0]), k = Integer.parseInt(s[1]);
        // long full = calc((long)n, (long)n, (long)k);
        // double discriminant = (2*k+1)*(2*k+1) + 4 * (full + 2);
        // double plus = (-2*k-1 + Math.sqrt(discriminant)) / 2.0, minus = (-2*k-1 - Math.sqrt(discriminant)) / 2.0;
        // ArrayList<Long> maybe = new ArrayList<>();
        // maybe.add((long)Math.floor(plus));
        // maybe.add((long)Math.floor(plus)-1);
        // maybe.add((long)Math.ceil(plus));
        // maybe.add((long)Math.ceil(plus)+1);
        // maybe.add((long)Math.floor(minus));
        // maybe.add((long)Math.floor(minus)-1);
        // maybe.add((long)Math.ceil(minus));
        // maybe.add((long)Math.ceil(minus)+1);
        // long ans = Long.MAX_VALUE/2;
        // for (long i : maybe) {
        //     if (1<=i && i<=n) {
        //         ans = Math.min(ans, calc(i, (long)n, (long)k));
        //     }
        // }
        // pw.println(ans);
        ...
        String[] s = br.readLine().split(" ");
        double n = Integer.parseInt(s[0]), k = Integer.parseInt(s[1]);
        long l = 1, r = (long)n-1, idx = -1;
        while (l <= r) {
            long i = (l + r) / 2;
            long prefix = prefix(i, (long)n, (long)k);
            long suffix = suffix(i, (long)n, (long)k);
            if (prefix <= suffix) {
                l = i+1;
                idx = i;
            } else {
        ...
        long ans = Math.abs(prefix(idx, (long)n, (long)k) - suffix(idx, (long)n, (long)k));
        if (idx < n-1) {
            ans = Math.min(ans, Math.abs(prefix(idx+1, (long)n, (long)k) - suffix(idx+1, (long)n, (long)k)));
        }
        pw.println(ans);
    }
