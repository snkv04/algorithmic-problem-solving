...
public class Forked {
    ...
    static void solve() throws IOException {
        ...
        int a = Integer.parseInt(asdf[0]), b = Integer.parseInt(asdf[1]);
        asdf = br.readLine().split(" ");
        int x1 = Integer.parseInt(asdf[0]), y1 = Integer.parseInt(asdf[1]);
        asdf = br.readLine().split(" ");
        int x2 = Integer.parseInt(asdf[0]), y2 = Integer.parseInt(asdf[1]);
        int[] dx = new int[] {a,b,a,b,-a,-b,-a,-b};
        int[] dy = new int[] {b,a,-b,-a,b,a,-b,-a};
        int ans = 0;
        for (int i = 0; i <= 7; i++) {
            int x3 = x1+dx[i];
            int y3 = y1+dy[i];
            ...
            if (Math.min(c,d)==Math.min(a,b) && Math.max(c,d)==Math.max(a,b))ans++;
        }
        if (a==b) ans /= 2;
        pw.println(ans);
    }
