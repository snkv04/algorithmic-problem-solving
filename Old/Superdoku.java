...
public class Superdoku {
    ...
    static void solve() throws IOException {
        /*
         * we know that the firs k rows are filled in
         * 
         * first make a grid. fill it in, and then subtract 1 from all of the first k rows' elements
         * check if it's valid so far. to do that, keep track of available rows and columns for every num.
         * fill in each element, but if the current row and col aren't available, then done.
         * 
         * for each column , go through every row:
         *      we know that there are certain remaining things to fill in. track that from teh start too for
         *      current col. basically, to simplify it, we are going to store the available vals for each ow
         *          and col when cehcking the firs tk rows. however, for each val, we are going to store the
         *          available rows for the remaining stuff
         * assume everythings fine now. for the remaining n-k rows, starting at n-k and going to n-1:
         *      f
         * 
         * 
         * for each column:
         *      go through the available numbers left to fill in. for each of thsoe actual elements:
         *          bsaically, put it in the first available row for that given element, and remove that
         *          row from the available rows for that element
         */
        ...
        HashSet<Integer>[] availForRow = new HashSet[n], availForCol = new HashSet[n];
        for (int i = 0; i < n; i++) {
            availForCol[i] = new HashSet<>();
            availForRow[i] = new HashSet<>();
            for (int j = 0; j < n; j++) {
                availForCol[i].add(j); availForRow[i].add(j);
            }
        }

        for (int i = 0; i < k; i++) {
            for (int j = 0; j < n; j++) {
                if (!availForRow[i].contains(grid[i][j]) || !availForCol[j].contains(grid[i][j])) {
                    pw.println("i="+i+", j="+j+", stopping");
                    pw.println("no");
                    return;
                }
                availForRow[i].remove(grid[i][j]);
                availForCol[j].remove(grid[i][j]);
            }
        }
        pw.println("yes");

        // TreeSet<Integer>[] availRows = new TreeSet[n];
        // for (int i = 0; i < n; i++) {
        //     availRows[i] = new TreeSet<>();
        //     for (int row = n-k; row < n; row++) {
        //         availRows[i].add(row);
        //     }
        // }
        // pw.println("availrows[0] = "+availRows[0]);

        // for (int j = 0; j < n; j++) {
        //     ArrayList<Integer> remaining = new ArrayList<>(availForCol[j]);
        //     for (int i = n-k; i < n; i++) {
        //         int element = remaining.get(i-(n-k));
        //         for (int l = i; l < n; l++) {
        //             if (availForRow[l].contains(element)) {
        //                 grid[l][j] = element;
        //                 availForRow[l].clear();
        //                 break;
        //             }
        ...
        //     // ArrayList<Integer> remaining = new ArrayList<>(availForCol[j]);
        //     // pw.println("remaining for col "+j+" = "+remaining);
        //     // int i = n-k;
        //     // for (int element : remaining) {
        //     //     int firstRow = availRows[element].ceiling(i);
        //     //     pw.println("col="+j+", element="+element+", row chosen = "+firstRow);
        //     //     availRows[element].remove(firstRow);
        //     //     grid[firstRow][j] = element;
        //     //     // grid[i][avail]
        //     //     i++;
        //     // }
        // }
        ...
        /*
         * then we can go row by row
         * 
         * for each row:
         *      for the first element, pick something remining from teh column and remove it
         *      from the frist column's remaining. also, add it to used.
         *      for the second, find the first element that was not used in this row.
         *      remove it from teh second column, and add it to used for row.
         * 
         *      etc...
         */
        for (int i = n-k; i < n; i++) {
            HashSet<Integer> usedForRow = new HashSet<>();
            for (int j = 0; j < n; j++) {
                int usingForCol = -1;
                for (int element : availForCol[j]) {
                    if (!usedForRow.contains(element)) {
                        usingForCol = element;
                        break;
                    }
                }
                usedForRow.add(usingForCol);
                availForCol[j].remove(usingForCol);
                grid[i][j] = usingForCol;
            }
        }
