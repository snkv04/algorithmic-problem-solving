...
public class BalancedString {
    static int[] left1, right1;
    static int num10, num01, n;
    static char[]s;
    ...
    public static void main(String[] args) throws IOException {
        ...
        n = s.length;
        build();
        int diff = (num10 - num01) / 2;
        // if (diff == 0) pw.print(0);
        // else if (diff > 0) {
        //     //
        // } else {
        //     //
    ...
    static void build() {
        ...
        left1[0] = s[0]=='1'?1:0;
        for (int i = 1; i <= n-1; i++) left1[i] = left1[i-1] + (s[i]=='1'?1:0);
        right1[n-1] = s[n-1]=='1'?1:0;
        for (int i = n-2; i >= 0; i--) right1[i] = right1[i+1] + (s[i]=='1'?1:0);

        num10 = 0;
        for (int i = 0; i <= n-1; i++) if (s[i]=='0') num10 += left1[i];
        num01 = 0;
        for (int i = 0; i <= n-1; i++) if (s[i]=='0') num01 += right1[i];
    }
