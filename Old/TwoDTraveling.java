    ...
    public static void main(String[] args) throws IOException {
        ...
        while (tc-->0) {
            ...
            for (int i = 0; i <= n-1; i++) {
                ...
                cities[i][0] = Integer.parseInt(line[0]);
                cities[i][1] = Integer.parseInt(line[1]);
            }

            long ans = Math.abs(cities[b][0]-cities[a][0]) + Math.abs(cities[b][1] - cities[a][1]);
            if (k > 1) {
                long closeAx = cities[0][0];
                long closeAy = cities[0][1];
                for (int i = 1; i <= k-1; i++) {
                    if (Math.abs(cities[i][0]-cities[a][0]) + Math.abs(cities[i][1] - cities[a][1])
                    < Math.abs(closeAx-cities[a][0]) + Math.abs(closeAy - cities[a][1])) {
                        closeAx = cities[i][0];
                        closeAy = cities[i][1];
                    }
                }
                long closeBx = cities[0][0], closeBy = cities[0][1];
                for (int i = 1; i <= k-1; i++) {
                    if (Math.abs(cities[i][0]-cities[b][0]) + Math.abs(cities[i][1] - cities[b][1])
                    < Math.abs(closeBx-cities[b][0]) + Math.abs(closeBy - cities[b][1])) {
                        closeBx = cities[i][0];
                        closeBy = cities[i][1];
                    }
                }
