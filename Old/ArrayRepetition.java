...
public class ArrayRepetition {
    ...
    static void solve() throws IOException {
        ...
        int q = Integer.parseInt(asdf[1]);
        ops = new long[n+1][3];
        // ops[i][0] is the type of operation i,
        // ops[i][1] is the argument of the operation,
        // and ops[i][2] is the length of the array after the operation
        boolean overflow = false; lastOperation = n;
        for (int i = 1; i <= n; i++) {
            asdf = br.readLine().split(" ");
            if (overflow) continue;
            long type = Integer.parseInt(asdf[0]), num = Integer.parseInt(asdf[1]);
            ops[i][0] = type;
            ops[i][1] = num;
            if (type == 1) {
                ops[i][2] = ops[i-1][2] + 1;
            } else {
                if ((long)1e18 / ops[i-1][2] < num+1) {
                    long multiplier = ((long)1e18 / ops[i-1][2]) + 1;
                    ops[i][2] = ops[i-1][2] * multiplier;
                    overflow = true;
                    lastOperation = i;
                    continue;
                }
                ops[i][2] = ops[i-1][2] * (num + 1);
            }
        }
        // pw.println();
        // for (int i = 0; i <= n; i++) pw.println(Arrays.toString(ops[i]));

        long[] queries = Arrays.stream(br.readLine().split(" ")).mapToLong(Long::parseLong).toArray();
        for (long query : queries) {
            int index = find(query);
            while (ops[index][0] == 2) {
                long mod = ops[index-1][2];
                query--;
                query %= mod;
                query++;
                index = find(query);
            }
            pw.print(ops[index][1]+" ");
        }
        pw.println();
    }

    static int find(long index) {
        int l = 1, r = lastOperation, op = -1;
        while (l <= r) {
            int mid = (l + r) / 2;
            if (ops[mid][2] >= index) {
                op = mid;
