...
public class QueriesForTheArray {
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            int n = s.length;
            int ls = 0, eu = Integer.MAX_VALUE, length = 0; // "latest sorted", "earliest unsorted"
            boolean works = true;
            for (int i = 0; i <= n-1; i++) {
                if (s[i]=='0') {
                    if (length <= ls) {
                    ...
                    if (length <= 1 || length < ls) ls = length;
                    if (length < eu) eu = Integer.MAX_VALUE;
                }
            }
