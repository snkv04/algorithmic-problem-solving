...
public class EasyAsABC {
    ...
    static void solve() throws IOException {
        ...
        for (int i = 0; i <= 2; i++) grid[i] = br.readLine().toCharArray();
        ArrayList<Point> points = new ArrayList<>();
        for (int i = 0; i <= 2; i++) for (int j = 0; j <= 2; j++) points.add(new Point(i, j));
        ...
        for (int i = 0; i <= 8; i++) {
            for (int j = i+1; j <= 8; j++) {
                for (int k = j+1; k <= 8; k++) {
                    ...
                    if ((Math.abs(a.i-b.i)<=1&&Math.abs(a.j-b.j)<=1) && (Math.abs(b.i-c.i)<=1&&Math.abs(b.j-c.j)<=1)) {
                        String str = grid[a.i][a.j]+""+grid[b.i][b.j]+grid[c.i][c.j];
                        if (str.compareTo(ans) < 0) ans = str;
                        str = ""+grid[c.i][c.j]+grid[b.i][b.j]+grid[a.i][a.j];
                        if (str.compareTo(ans) < 0) ans = str;
                    }
                }
            }
