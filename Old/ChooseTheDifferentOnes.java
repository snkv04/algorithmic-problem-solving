...
public class ChooseTheDifferentOnes {
    ...
    static void solve() throws IOException {
        ...
        int n = Integer.parseInt(asdf[0]), m = Integer.parseInt(asdf[1]), k = Integer.parseInt(asdf[2]);
        int[] a=  Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        int[] b=  Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        HashSet<Integer> c = new HashSet<>(), d = new HashSet<>();
        for (int num : a) c.add(num);
        for (int num : b) d.add(num);

        int countBoth = 0, countA = 0, countB = 0;
        for (int i = 1; i <= k; i++) {
        ...
        if (countA>k/2 || countB > k/2) pw.println("NO");
        else if (countA + countB + countBoth < k) pw.println("NO");
        else pw.println("YES");
    }
