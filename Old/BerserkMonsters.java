...
public class BerserkMonsters {
    ...
    static void solve() throws IOException {
        ...
        int[] a = new int[n+2];
        for (int i = 1; i <= n; i++) a[i] = Integer.parseInt(asdf[i-1]);
        asdf = br.readLine().split(" ");
        int[] d = new int[n+2];
        d[0] = d[n+1] = 2000000005;
        for (int i = 1; i <= n; i++) d[i] = Integer.parseInt(asdf[i-1]);
        // pw.println(Arrays.toString(a));
        // pw.println(Arrays.toString(d));

        // int[] next = new int[n+2], prev = new int[n+2];
        // for (int i = 1; i <= n; i++) { next[i] = i+1; prev[i] = i-1; }
        // int[] receiving = new int[n+2];
        // TreeSet<Integer> died = new TreeSet<>(); for (int i = 1; i <= n; i++) died.add(i);
        // for (int i = 1; i <= n; i++) {
        //     TreeSet<Integer> newDied = new TreeSet<>();
        //     receiving = new int[n+2];
        //     for (int j : died) {
        //         if (i != 1) {
        //             next[prev[j]] = next[j];
        //             prev[next[j]] = prev[j];
        //         }
        //         if (i != 1) receiving[prev[j]] -= a[j];
        //         receiving[prev[j]] += a[next[j]];
        //         if (i != 1) receiving[next[j]] -= a[j];
        //         receiving[next[j]] += a[prev[j]];

        //         if (receiving[prev[j]] > d[prev[j]]) {
        //             newDied.add(prev[j]);
        //         }
        //         if (receiving[next[j]] > d[next[j]]) {
        //             newDied.add(next[j]);
        //         }
        //     }
        //     pw.println("receiving on round "+i+": "+Arrays.toString(receiving));

        //     pw.print(newDied.size()+" ");
        //     died = newDied;
        // }
        // pw.println();
        ...
        int[] next = new int[n+2], prev = new int[n+2];
        for (int i = 1; i <= n; i++) { next[i] = i+1; prev[i] = i-1; }
        HashSet<Integer> matter = new HashSet<>(); for (int i = 1; i <= n; i++) matter.add(i);
        for (int i = 1; i <= n; i++) {
            // if (matter.isEmpty()) {
            //     pw.print("0 ");
            //     continue;
            // }
            HashSet<Integer> newMatter = new HashSet<>();
            ArrayList<Integer> died = new ArrayList<>();
            for (int index : matter) {
                ...
                if (damage > d[index]) {
                    died.add(index);
                    newMatter.add(next[index]);
                    newMatter.add(prev[index]);
                }
            }
            // pw.println("i = "+i);
            // pw.println(Arrays.toString(damage));
            // pw.println(died);
            // pw.println(newMatter);

            pw.print(died.size()+" ");
            for (int index : died) {
                int after = next[index], before = prev[index];
                prev[after] = before;
                next[before] = after;
                newMatter.remove(index);
            }
            newMatter.remove(0);
            newMatter.remove(n+1);
            matter = newMatter;
        }
        // pw.println("\n");
        pw.println();
    ...
    static class Triple {
        ...
        public Triple(int i, int a, int d) {
            this.i = i;
            this.a = a;
            this.d = d;
        }
