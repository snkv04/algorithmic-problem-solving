...
public class CodeNames {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i <= n-1; i++) {
            pw.println("i = "+i+", adjList = "+adjLists[i]);
        }
        pw.println();

        match = new int[n]; for (int i = 0; i <= n-1; i++) match[i] = -1;
        for (int i = 0; i <= n-1; i++) {
            ...
            //     match[i] = -1;
            // }
            pw.println("i="+i+", finished dfs, match looks like: "+Arrays.toString(match));
        }
        // pw.println(Arrays.toString(match));

        int maxSwapFree = 0;
        for (int i = 0; i <= n-1; i++) {
            if (match[i] == -1) maxSwapFree++;
        }
        pw.println(maxSwapFree + (n-maxSwapFree)/2);
