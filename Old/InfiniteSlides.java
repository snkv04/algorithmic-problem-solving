...
public class InfiniteSlides {
    ...
    static void solve() throws IOException {
        ...
        double l = 0.0, r = 1e9, ans = -1;
        for (int i = 1; i <= 100; i++) {
            double mid1 = l + (r-l)/3.0, mid2 = r - (r-l)/3.0;
            if (dist(mid1, w) < dist(mid2, w)) {
                r = mid2;
            } else {
                l = mid1;
            }
    ...
    static double dist(double t, double w) {
        double dx = Math.cos(t+w) - (Math.cos(t) + 1);
        double dy = Math.sin(t+w) - Math.sin(t);
        double dz = w - t;
