...
public class BinaryStringToSubsequences {
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            TreeSet<Integer> unused = new TreeSet<>(); for (int i = 1; i <= n; i++) unused.add(i);
            TreeSet<Integer> waitingFor0 = new TreeSet<>(), waitingFor1 = new TreeSet<>();
            int[] ans = new int[n];
            for (int i = 0; i <= n-1; i++) {
                int subsequence = -1;
                if (binary[i]=='0') {
                    if (waitingFor0.isEmpty()) {
                        subsequence = unused.pollFirst();
