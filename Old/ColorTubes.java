...
public class ColorTubes {
    ...
    static Stack<Integer>[] tubes;
    static ArrayList<Integer> emptySpaces;
    ...
    static void solve() throws IOException {
        ...
        for (int i = 0; i <= n; i++) {
            ...
            tubes[i] = new Stack<>();
            for (int num : line) if (num != 0) tubes[i].add(num); else emptySpaces.add(i);
        }
        for (int i = 0; i <= n; i++) pw.println(tubes[i]);
        pw.println(emptySpaces);
        
    }

    static void move(int from, int to) {
        emptySpaces.remove(emptySpaces.indexOf(to));
        emptySpaces.add(from);
        tubes[to].add(tubes[from].pop());
    }
