...
public class WoodenToyFestival {
    ...
    public static boolean check(int maxTime) {
        int end = b[0] + 2*maxTime;
        int i = 0;
        while (i+1<=n-3 && b[i+1] <= end) {
            i++;
        }
        int start = b[n-1] - 2*maxTime;
        int j = n-1;
        while (j-1 >= i && b[j-1] >= start) {
            j--;
        }
        if (i==j) return true;
        else return b[j-1]-b[i+1]<=2*maxTime;
    }
