...
public class Dunglish {
    ...
    static void solve() throws IOException {
        /*
         * map each word itself to num of correct and incorrect
         * convert the array of words to an array of C and I counts
         * 
         * for correct, just use multiplication principle
         * for incorrect, go through all subsets (so the power set), minus empty set
         * for each subset:
         *      if any of those selected have 0 incorrect, move to next subset
         *      otherwise, continue
         *      for those selected, multiply only number of incorrect
         *      for remaining ones, multiply only number of correct
         *      add product to overall sum of incorrect (and print after loop)
         */

        int n = Integer.parseInt(br.readLine());
        String[] split = br.readLine().split(" ");
        HashMap<String, TreeSet<String>> translations = new HashMap<>(), mistranslations = new HashMap<>();
        int m = Integer.parseInt(br.readLine());
        while (m-->0) {
            String[] translation = br.readLine().split(" ");
            if (translation[2].charAt(0) == 'c') {
                if (!translations.containsKey(translation[0])) {
                    translations.put(translation[0], new TreeSet<>());
                }
                translations.get(translation[0]).add(translation[1]);
            } else {
                if (!mistranslations.containsKey(translation[0])) {
                    mistranslations.put(translation[0], new TreeSet<>());
                }
                mistranslations.get(translation[0]).add(translation[1]);
            }
        }
        // pw.println(translations.mset);
        // pw.println(mistranslations.mset);

        long[][] counts = new long[n][2];
        for (int i = 0; i <= n-1; i++) {
            if (translations.containsKey(split[i])) {
                counts[i][0] = translations.get(split[i]).size();
            }
            if (mistranslations.containsKey(split[i])) {
                counts[i][1] = mistranslations.get(split[i]).size();
            }
        }
        // for (int i = 0; i <= n-1; i++) pw.println("word = "+split[i]+"; counts = "+Arrays.toString(counts[i]));

        long correct = counts[0][0], incorrect = 0;
        for (int i = 1; i <= n-1; i++) {
            correct *= counts[i][0];
        }
        for (int i = 1; i < (1<<(n)); i++) {
            ...
            for (int j = 0; j <= n-1; j++) {
                boolean included = ((1<<j)&i) > 0; // included in the set of incorrect words
                if ((included&&(counts[j][1]==0)) || ((!included) && (counts[j][0]==0))) {
                    willWork = false;
                    break;
                }
            }

            if (!willWork) continue;

            long product = 1;
            for (int j = 0; j <= n-1; j++) {
                boolean included = ((1<<j)&i) > 0;
                if (included) {
                    product *= counts[j][1];
                } else {
                    product *= counts[j][0];
        ...
        /*
         * cases to check for expected functionality:
         *      c > 1 && i > 1 (normal)
         *      c > 1 && i == 1
         *      c > 1 && i == 0
         *      c == 1 && i > 1
        ...
        if (correct + incorrect > 1) {
            ...
            boolean onlyIncorrect = false;
            for (int i = 0; i <= n-1; i++) {
                if (i >= 1) translation.append(" ");
                if (translations.containsKey(split[i])) {
                    translation.append(translations.get(split[i]).first());
                } else {
                    onlyIncorrect = true;
                    translation.append(mistranslations.get(split[i]).first());
                }
            }
            
            pw.println(translation.toString());
            pw.println(onlyIncorrect?"incorrect":"correct");
        }
    }
