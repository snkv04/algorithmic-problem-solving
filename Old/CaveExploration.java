...
public class CaveExploration {
    ...
    static int[][] grid;
    static int n;
    static int[][] newGrid; static boolean[][] visited;
    ...
    static boolean check(int highest) {
        newGrid = new int[n][n];
        for (int i = 0; i <= n-1; i++) {
            for (int j = 0; j <= n-1; j++) {
                newGrid[i][j] = grid[i][j] > highest ? -1 : grid[i][j];
            }
        }
