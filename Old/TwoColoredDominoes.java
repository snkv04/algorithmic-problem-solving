...
public class TwoColoredDominoes {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            for (int i = 0; i <= n-1; i++) Arrays.fill(newBoard[i], '.');
            boolean works = true;
            for (int i = 0; i <= n-1 && works; i++) {
                int black=0, white = 0, up = 0;
                for (int j = 0; j <= m-1; j++) {
            ...
            for (int j = 0; j <= m-1 && works; j++) {
                int black=0, white = 0, left = 0;
                for (int i = 0; i <= n-1; i++) {
                    if (board[i][j]=='B') black++;
                    if (board[i][j]=='W') white++;
                    if (board[i][j]=='L') left++;
            ...
            if (!works && check(newBoard)) pw.println(-1);
            else for (char[] row : newBoard) pw.println(row);
        }
