...
public class TrainingBeforeOlympiad {
    ...
    static void solve() throws IOException {
        ...
        long[] prefsums = new long[n+1]; int[] odds = new int[n+1];
        for (int i = 1; i <= n; i++) {
            prefsums[i] = prefsums[i-1] + a[i-1];
            odds[i] = odds[i-1] + (a[i-1]%2==1?1:0);
        }
        // pw.println(Arrays.toString(prefsums));
        // pw.println(Arrays.toString(odds));

        pw.print(prefsums[1]+" ");
        for (int k = 2; k <= n; k++) {
            long result = prefsums[k];
            result -= (odds[k]/6)*2;

            if (odds[k]%6==1 || odds[k]%6==3 || odds[k]%6==5) result -= 1;
            else if (odds[k]%6 == 4) result -= 2;
            pw.print(result+" ");
        }
        pw.println();
    }
