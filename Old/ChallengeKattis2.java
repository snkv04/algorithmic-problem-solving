...
public class ChallengeKattis2 {
    ...
    static void solve() throws IOException {
        ...
        int[] starts = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        int[] ends = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        Multiset diffs = new Multiset();
        for (int end : ends) {
            for (int start : starts) {
                if (start > end) break;
                else diffs.add(end-start);
            }
        }
        // pw.println(diffs.mset);

        int best = 0, bestFrequency = -1;
        for (Map.Entry<Integer, Integer> entry : diffs.mset.entrySet()) {
            if (entry.getValue() > bestFrequency) {
