...
public class StampCombinations {
    ...
    static void solve() throws IOException {
        ...
        int m = Integer.parseInt(asdf[0]), n = Integer.parseInt(asdf[1]);
        int[] a = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        long[] pref = new long[m+1];
        for (int i = 1; i <= m; i++) {
            pref[i] = pref[i-1] + a[i-1];
        }
        HashSet<Long> suffixes = new HashSet<>();
        long suf = 0;
        suffixes.add((long)0);
        for (int i = m; i >= 1; i--) {
            suf += a[i-1];
        ...
        while (n-->0) {
            ...
            boolean works = false;;
            for (int i = 0; i <= m; i++) {
                // pw.println("chekcing suffixes for: " + (query-pref[i]));
                if (suffixes.contains(query-pref[i])) {
                    works = true; break;
                }
            }
