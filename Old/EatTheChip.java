...
public class EatTheChip {
    ...
    static void solve() throws IOException {
        ...
        int h = Integer.parseInt(asdf[0]), w = Integer.parseInt(asdf[1]),
            i1 = Integer.parseInt(asdf[2]), j1 = Integer.parseInt(asdf[3]),
            i2 = Integer.parseInt(asdf[4]), j2 = Integer.parseInt(asdf[5]);
        if (i1 >= i2) {
            pw.println("Draw");
            return;
        }
        ...
        if (dist % 2 == 0) {
            int halfDist = dist / 2;
            if (j1==j2) pw.println("Bob");
            else if (j1 < j2) {
                j1 -= halfDist;
                j1 = Math.max(j1, 1);
                if (j2-j1 <= halfDist) pw.println("Bob");
                else pw.println("Draw");
            } else {
                j1 += halfDist;
                j1 = Math.min(j1, w);
                if (j1-j2 <= halfDist) pw.println("Bob");
                else pw.println("Draw");
            }
        } else {
            if (j1==j2) pw.println("Alice");
            else if (j1 < j2) {
                i1++;
                j1 += 1; j1 = Math.min(j1, w);
                int halfDist = (i2-i1) / 2;
                j2 = Math.min(w, j2 + halfDist);
                if (j2-j1 > halfDist) pw.println("Draw");
                else pw.println("Alice");
            } else {
                i1++;
                j1--; if (j1 < 1) j1 = 1;
                int halfDist = (i2-i1) / 2;
                j2 = Math.max(1, j2 - halfDist);
                if (j1-j2 > halfDist) pw.println("Draw");
                else pw.println("Alice");
            }
        }
    }
