...
public class GoodKeyBadKey {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            // if (n == 1) {
            //     pw.println(Math.max(a[0]-k, a[0]>>1));
            //     continue;
            // }
            ...
            // long max = 0;
            // for (int i = -1; i <= n-1; i++) {
            //     long sum = i==-1?0:pref[i];
            //     for (int j = 1; j <= 35 && j+i<=n-1; j++) {
            //         sum += a[i+j] >> j;
            //     }
            ...
            // the code above is just a shorter version of this
            // (except for the fact that this doesn't work)
            long[][] memo = new long[n+1][36];
            for (int i = 1; i <= n; i++) {
                memo[i][0] = memo[i-1][0] + a[n-1] - k;
            }
            for (int i = 1; i <= n; i++) {
                for (int j = 1; j <= 35; j++) {
                    memo[i][j] = Math.max(memo[i-1][j-1] + (a[i-1]>>(long)j), memo[i-1][j] - k + (a[i-1]>>((long)j)));
                }
            }
            // pw.println(memo[n][35]);
            long max = 0;
            for (int j = 0; j <= 35; j++) {
                max = Math.max(max, memo[n][j]);
            }
            pw.println(max);
        }
