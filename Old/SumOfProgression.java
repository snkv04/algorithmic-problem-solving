...
public class SumOfProgression {
    ...
    static void solve() throws IOException {
        ...
        int n = Integer.parseInt(asdf[0]), q = Integer.parseInt(asdf[1]);
        long[]a = Arrays.stream(br.readLine().split(" ")).mapToLong(Integer::parseInt).toArray();
        int maxd = (int) sqrt(q);
        long[][] pref = new long[maxd+1][n+1], pref2 = new long[maxd+1][n+1];
        for (int step = 1; step <= maxd; step++) {
            for (int i = 1; i <= n; i++) {
                pref[step][i] = pref[step][Math.max(0, i-step)] + a[i-1];
                long block = ((i-1)/step) + 1;
                pref2[step][i] = pref2[step][Math.max(0, i-step)] + block * a[i-1];
            }
        }
        // for (int i = 1; i <= maxd; i++) {
        //     pw.println("step = "+i);
        //     pw.println("pref["+i+"] = "+Arrays.toString(pref[i]));
        //     pw.println("pref2["+i+"] = "+Arrays.toString(pref2[i]));
        // }

        while (q-->0) {
            asdf = br.readLine().split(" ");
            int s = Integer.parseInt(asdf[0]), d = Integer.parseInt(asdf[1]), k = Integer.parseInt(asdf[2]);
            // k elements, starting with s and taking steps of d,
            // multiplying each by its index in the chosen sequence
            int l = s, r = s + d*(k-1);
            if (d > maxd) {
                s--;
                long ans = 0;
                ...
                long subarray = pref[d][r] - pref[d][Math.max(0, l-d)];
                long subarray2 = pref2[d][r] - pref2[d][Math.max(0, l-d)];
                int lblock = (l-1)/d + 1;
                long ans = subarray2 - (lblock-1) * subarray;
                pw.print(ans+" ");
            }
        }
        pw.println();
