...
public class HoneycombWalk {
    ...
    static int[] hexdi = {0, 0, -1, 1, -1, 1};
    static int[] hexdj = {-1, 1, 0, 0, -1, 1};
    static int[] ans;
    ...
    public static void main(String[] args) throws IOException {
        ...
        ans = new int[15];
        HashMap<Triple, Integer> memo = new HashMap<>();
        HashMap<Triple, Boolean> visited = new HashMap<>();
        Queue<Triple> q = new LinkedList<>();
        q.add(new Triple(0, 0, 0));
        memo.put(new Triple(0, 0, 0), 1);
        visited.put(new Triple(0, 0, 0), false);
        while (!q.isEmpty()) {
            Triple node = q.poll();
            ...
            if (node.steps <= 13) {
                for (int i = 0; i <= 5; i++) {
                    int newi = node.i + hexdi[i];
                    int newj = node.j + hexdj[i];
                    Triple next = new Triple(newi, newj, node.steps + 1);
                    if (!visited.containsKey(next)) {
                        visited.put(next, false);
                        memo.put(next, 0);
                    }
                    memo.put(next, memo.get(next) + memo.get(node));
                    q.add(next);
                }
            }
        }
        for (int i = 2; i <= 14; i++) {
            ans[i] = memo.get(new Triple(0, 0, i));
        }
    ...
    static class Triple {
        ...
        public Triple(int i, int j, int steps) {
            this.i = i;
            this.j = j;
            this.steps = steps;
        }
        ...
        public boolean equals(Object obj) {
            ...
            return this.i == other.i && this.j == other.j && this.steps == other.steps;
        }

        // @Override
        public int hashCode() {
            return (i+" "+j+" "+steps).hashCode();
        }
    }
