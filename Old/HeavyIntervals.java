...
public class HeavyIntervals {
    ...
    static void solve() throws IOException {
        ...
        PriorityQueue<Long> c = new PriorityQueue<>(Comparator.comparingLong(o -> -o));
        for (String s : il) l.add(Integer.parseInt(s));
        il = br.readLine().split(" ");
        for (String s : il) r.add(Integer.parseInt(s));
        il = br.readLine().split(" ");
        for (String s : il) c.add((long)Integer.parseInt(s));

        PriorityQueue<Long> lengths = new PriorityQueue<>();
        for (int right : r) {
            int left = l.lower(right);
            lengths.add((long)(right-left));
            l.remove(left);
        }

        long sum = 0;
        while (!lengths.isEmpty()) {
            long length = lengths.poll();
            sum += length * c.poll();
        }
        pw.println(sum);
    }
