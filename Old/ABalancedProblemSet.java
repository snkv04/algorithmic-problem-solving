...
public class ABalancedProblemSet {
    ...
    static void solve() throws IOException {
        ...
        long x = Integer.parseInt(asdf[0]), n = Integer.parseInt(asdf[1]);
        HashSet<Long> _factors = new HashSet();
        long sqrt = sqrt(x);
        for (long i = 1; i <= Math.min(x - i * (n-1), sqrt); i++) {
            if (x%i==0) {
                _factors.add(i);
                _factors.add(x/i);
            }
        ...
        for (long factor : _factors) factors.add(factor);
        Collections.sort(factors);
        // pw.println(factors);
        int l = 0, r = factors.size()-1, ans = -1;
        while (l <= r) {
            int mid = (l + r) / 2;
            long f = factors.get(mid);
            if (x % f == 0 && x > f * (n-1)) {
                ans = mid;
                l = mid+1;
            } else {
        ...
        pw.println(ans==-1?1:factors.get(ans));
    }
