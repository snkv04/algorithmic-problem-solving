...
public class HelmetsInNightLight {
    ...
    static void solve() throws IOException {
        ...
        int n = Integer.parseInt(asdf[0]), p = Integer.parseInt(asdf[1]);
        int[] a = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        int[] b = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        PriorityQueue<Person> pq = new PriorityQueue<>(Comparator.comparingInt(o -> o.cost)),
            pq2 = new PriorityQueue<>(Comparator.comparingInt(o -> o.cost));
        for (int i = 0; i <= n-1; i++) {
            pq2.add(new Person(i, a[i], b[i]));
        }

        long ans = p;
        pq.add(pq2.poll());
        while (!pq2.isEmpty() && pq.peek().cost <= p) {
            Person using = pq.poll();
            int size = pq2.size();
            for (int i = 1; i <= Math.min(size, using.max); i++) {
                pq.add(pq2.poll());
                ans += using.cost;
            }
        }
    ...
    static class Person {
        ...
        public Person(int ind, int max, int cost) {
            this.ind = ind;
            this.max = max;
            this.cost = cost;
        }
