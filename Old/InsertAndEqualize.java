...
public class InsertAndEqualize {
    ...
    static void solve() throws IOException {
        ...
        for (int num : a) if (Math.abs(num-max)!=0) dists.add(Math.abs(num-max));
        int gcd=-1;
        for (int dist : dists) {gcd=dist;break;}
        for (int dist : dists) {gcd = gcd(gcd, dist);}

        long ans = 0;
        for (int i = 1; i <= n; i++) {
            if (!dists.contains(gcd*i)) {
                ans = i;
                break;
            }
        }
