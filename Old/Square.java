...
public class Square {
    ...
    static void solve() throws IOException {
        int mxx = Integer.MIN_VALUE, mnx = Integer.MAX_VALUE,  mxy = Integer.MIN_VALUE, mny = Integer.MAX_VALUE;
        for (int i=  0; i <= 3; i++) {
            String[] asdf = br.readLine().split(" ");
            int x = Integer.parseInt(asdf[0]), y = Integer.parseInt(asdf[1]);
            mxx = Math.max(mxx, x);
            mnx = Math.min(mnx, x);
            mxy = Math.max(mxy, y);
            mny = Math.min(mny, y);
        }
        pw.println((mxx-mnx)*(mxy-mny));
    }
