...
public class Generators {
    ...
    static void solve() throws IOException {
        ...
        PriorityQueue<Edge> edges = new PriorityQueue<Edge>(Comparator.comparingInt(e -> e.w));
        while (m-->0) {
            tokens = br.readLine().split(" ");
            int city = Integer.parseInt(tokens[0])-1, cost = Integer.parseInt(tokens[1]);
            edges.add(new Edge(city, n, cost));
        }
        tokens = br.readLine().split(" ");
        for (int i = 0; i < n; i++) {
            edges.add(new Edge(i, (i+1)%n, Integer.parseInt(tokens[i])));
        }
