...
public class MischeviousShooter {
    ...
    static void solve() throws IOException {
        ...
        k = Integer.parseInt(asdf[2]);
        grid = new char[n][m];
        for (int i = 0; i <= n-1; i++) {
            char[] line = br.readLine().toCharArray();
            for (int j = 0; j <= m-1; j++) grid[i][j] = line[j];
        }
        
        int ans = 0;
        for (int i = 1; i <= 4; i++) {
            grid = rotate(grid);
            ans = Math.max(ans, solve2(grid));
        }
        pw.println(ans);
    }

    static char[][] rotate(char[][] orig) {
        int r = orig.length, c = orig[0].length;
        // pw.println("r="+r+", c="+c);
        char[][] grid = new char[c][r];
        for (int i = 0; i <= c-1; i++) {
            for (int j = 0; j <= r-1; j++) {
                grid[i][j] = orig[j][c-1-i];
            }
    ...
    static int solve2(char[][] grid) {
        ...
        int[][] horiz = new int[r+2][c+2], downleft = new int[r+2][c+2];
        for (int i = 1; i <= r; i++) {
            for (int j = 1; j <= c; j++) {
                horiz[i][j] = horiz[i][j-1] + (grid[i-1][j-1]=='#'?1:0);
                downleft[i][j] = downleft[i-1][j+1] + (grid[i-1][j-1]=='#'?1:0);
            }
            // pw.println(Arrays.toString(grid[i-1]));
        }
        // pw.println("horiz = ");
        // for (int i = 1; i <= r; i++) {
        //     pw.println(Arrays.toString(horiz[i]));
        // }
        // pw.println("downleft = ");
        // for (int i = 1; i <= r; i++) {
        //     pw.println(Arrays.toString(downleft[i]));
        // }
        // pw.println();

        int[][] memo = new int[r+1][c+1];
        for (int j = 1; j <= c; j++) {
            memo[1][j] = horiz[1][j] - horiz[1][Math.max(0, j-k-1)];
        }
        // pw.println(Arrays.toString(memo[0]));
        // pw.println(Arrays.toString(memo[1]));
        for (int i = 2; i <= r; i++) {
            for (int j = 1; j <= c; j++) {
                int val = memo[i-1][j];

                int fromi = i-1, fromj = j-k;
                if (fromj < 1) {
                    int dist = 1 - fromj;
                    fromj += dist;
                    fromi -= dist;
                }
                int toi = i-k-2, toj = j+1;
                if (fromi >= 1) {
                    val -= downleft[fromi][fromj];
                }
                if (toi >= 0) {
                    val += downleft[toi][toj];
                }

                val += horiz[i][j] - horiz[i][Math.max(0, j-k-1)];
                memo[i][j] = val;
            }
            // pw.println(Arrays.toString(memo[i]));
        }
        // pw.println();
