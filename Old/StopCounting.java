...
public class StopCounting {
    public static void main(String[] args) throws IOException {
        ...
        long[] prefsums = new long[n+1]; long[] sufsums = new long[n+1];
        for (int i = 1; i <= n; i++) prefsums[i] = prefsums[i-1] + a[i-1];
        for (int i = n-1; i >= 0; i--) sufsums[i] = sufsums[i+1] + a[i];
        double ans = 0;
        for (int i = 1; i <= n; i++) ans = Math.max(ans, prefsums[i] / ((double)i));
        for (int i = n-1; i >= 0; i--) ans = Math.max(ans, sufsums[i] / ((double)(n-i)));
        pw.println(ans);
