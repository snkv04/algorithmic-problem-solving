...
public class ShiftAndReverse {
    ...
    static void solve() throws IOException {
        ...
        int increase = 0, decrease = 0, min = Integer.MAX_VALUE;
        for (int i = 0; i <= n-1; i++) {
            if (a[i] < a[(i+1)%n]) increase++;
            else if (a[i] > a[(i+1)%n]) decrease++;
        ...
        if (increase==0 && decrease == 0) {
            ...
            if (decrease == 1) {
                ...
                if (start == 0) ans = 0;
                    ...
                    ans = Math.min(2+start, n-start);
                }
            }
            if (increase == 1) {
                int start = -1;
                for (int i = 0; i <= n-1; i++) {
                    if (a[i] < a[(i+1)%n]) start = i;
                }
                ans = Math.min(ans, 1 + Math.min(n-1-start, start+1));
            }
        }
        pw.println(ans);
    }
