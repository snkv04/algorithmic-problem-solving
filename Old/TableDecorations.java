...
public class TableDecorations {
    ...
    public static void main(String[] args) throws IOException {
        ...
        long r = Integer.parseInt(line[0]), g = Integer.parseInt(line[1]), b = Integer.parseInt(line[2]);
        long ans = Math.min(r, Math.min(g, b));
        r-=ans;
        g-=ans;
        b-=ans;
        ArrayList<Long> remaining = new ArrayList<>();
        if (r!=0) remaining.add(r);
        if (g!=0) remaining.add(g);
        if (b!=0) remaining.add(b);
        if (remaining.size()==2) {
            if (remaining.get(1) >= 2*remaining.get(0)) {
                ans += Math.min(remaining.get(0), remaining.get(1)/2);
            } else {
                long temp = Math.min(remaining.get(0)/3, remaining.get(1)/3);
                ans += (long)2*temp;
                remaining.set(0, remaining.get(0)-3*temp);
                remaining.set(1, remaining.get(1)-3*temp);
                Collections.sort(remaining);
                if (remaining.get(0)==1 && remaining.get(1)==2) ans++;
            }
        }
        pw.println(ans);
