...
public class DynamicRangeMinimumQueries {
    ...
    static class SegmentTree {
        // the original array is considered as 0-indexed
        // this default version is for doing range maximum queries
        ...
        public void build(int[] a, int node, int tmpL, int tmpR) {
            if (tmpL == tmpR) {
                ...
                f[node] = Math.min(f[2*node], f[2*node+1]); // computes the higher-level node afterward
            }
        }
        ...
        public void update(int ind, int newVal, int node, int tmpL, int tmpR) {
            if (tmpL == tmpR) {
                ...
                f[node] = Math.min(f[2*node], f[2*node+1]);
            }
        }
