...
public class TeaTasting {
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            int[] _a = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
            int[] a = new int[n+1]; for (int i = 1; i <= n; i++) a[i] = _a[i-1];
            int[] _b = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
            int[] b = new int[n+1]; for (int i = 1; i <= n; i++) b[i] = _b[i-1];
            long[] prefsums = new long[n+1];
            for (int i = 1; i <= n; i++) prefsums[i] = prefsums[i-1] + b[i];
            // pw.println(Arrays.toString(prefsums));

            int[] prefsums2 = new int[n+1];
            long[] drink = new long[n+1];
            for (int i = 1; i <= n; i++) {
                // long total = prefsums[n] - prefsums[i];
                int l = 0, r = n, j = -1; // 1-indexed
                while (l <= r) {
                    int mid = (l+r)/2;
                    if (prefsums[mid]-prefsums[i-1] <= a[i]) {
                        j = mid;
                        l = mid+1;
                    } else {
                        r = mid-1;
                ...
                if (a[i] - (prefsums[j] - prefsums[i-1]) > 0 && j < n) {
                    drink[j+1] += Math.min(b[j+1], a[i] - (prefsums[j] - prefsums[i-1]));
                }
                prefsums2[i]++;
                if (j < n) prefsums2[j+1]--;
            }
            for (int i = 1; i <= n; i++) {
                prefsums2[i] = prefsums2[i-1] + prefsums2[i];
            }
            for (int i = 1; i <= n; i++) {
                drink[i] += prefsums2[i] * (long) b[i];
            }
            for (int i = 1; i <= n; i++) pw.print(drink[i] + " ");
            pw.println();
        }
