...
public class LightUp {
    ...
    static void solve() throws IOException {
         ...
         * just use a char array lol
         * 
         * have a boolean array for lightbulb. have an
         * 
         * blocked cells can be easily checked. return immediately if any of them fail.
         * 
         * now, go through all cells. if we see a lightbulb:
         *      start from there. go left all the way, and mark all cells as lit.
         *          and if you see another lightbulb, then return. if you see a blcoked cell, then continue
         *      samae for up, down, and right.
         * 
         */
        ...
        ArrayList<Point> bulbs = new ArrayList<>();
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                int asInt = grid[i][j] - '0';
                if (asInt >= 0 && asInt <= 4) {
                    int adjacentBulbs = 0;
                    for (int k = 0; k <= 3; k++) {
                        int newi = i + di[k], newj = j + dj[k];
                        if (inBounds(newi, newj, n) && grid[newi][newj] == '?') {
                            adjacentBulbs++;
                        }
                    }
                    if (adjacentBulbs != asInt) {
                        pw.println(0);
                        return;
                    }
                }
                ...
                if (grid[i][j] == '?') {
                    bulbs.add(new Point(i, j));
                }
            }
        }
        // pw.println("bulbs = "+bulbs);

        boolean[][] lit = new boolean[n][n];
        for (Point bulb : bulbs) {
            int i = bulb.i, j = bulb.j;
            for (int k = 0; k <= 3; k++) {
                int newi = i, newj = j;
                while (inBounds(newi+di[k], newj+dj[k], n)) {
                    newi += di[k];
                    newj += dj[k];
                    if (grid[newi][newj] == '?') {
                        pw.println(0);
                        return;
                    } else if (grid[newi][newj] == 'X' || (grid[newi][newj]-'0'>=0 && grid[newi][newj]-'0'<=4)) {
                        break;
                    }
                    lit[newi][newj] = true;
                }
    ...
    static boolean inBounds(int i, int j , int n) {
