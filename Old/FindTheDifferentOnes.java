...
public class FindTheDifferentOnes {
    ...
    static void solve() throws IOException {
        ...
        TreeSet<Pair> segments = new TreeSet<Pair>(Comparator.comparingInt(o -> o.a));
        int l = 0;
        for (int i = 0; i <= n-1; i++) {
            if (a[i] == a[l]) {
                //
            } else {
                segments.add(new Pair(l, i-1));
                l = i;
            }
        }
        segments.add(new Pair(l, n-1));
        // pw.println(segments);

        int q = Integer.parseInt(br.readLine());
        while (q-->0) {
            String[] asdf = br.readLine().split(" ");
            l = Integer.parseInt(asdf[0])-1;
            int r = Integer.parseInt(asdf[1])-1;
            
            Pair floor = segments.floor(new Pair(l, r));
            if (floor.b >= r) {
                pw.println("-1 -1");
            } else {
                pw.println((l+1)+" "+(floor.b+2));
            }
        }
        // pw.println();
    }
