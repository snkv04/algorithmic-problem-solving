...
public class ArrayGame {
    ...
    static void solve() throws IOException {
        ...
        for (int i = 0; i <= n-1; i++) {
            nums.add(a[i]);
            minDiff = Math.min(minDiff, a[i]);
        }
        for (int i = 0; i <= n-1; i++) {
            for (int j = i+1; j <= n-1; j++) {
                long diff = Math.abs(a[i]-a[j]);
                minDiff = Math.min(minDiff, diff);
                if (k == 2) minDiff = Math.min(minDiff, Math.min(
                    nums.ceiling(diff)!=null?Math.abs(nums.ceiling(diff)-diff):Long.MAX_VALUE,
                    nums.floor(diff)!=null?Math.abs(nums.floor(diff)-diff):Long.MAX_VALUE
                ));
            }
        }
        pw.println(minDiff);
