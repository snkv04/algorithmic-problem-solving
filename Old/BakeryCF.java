...
public class BakeryCF {
    public static void main(String[] args) throws IOException {
        ...
        if (k==0) { pw.println(-1); pw.close(); return; }
        int[][] abw = new int[m][3];
        for (int i = 0; i <= m-1; i++) {
            abw[i] = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        }
        int[] storages = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        TreeSet<Integer> s = new TreeSet<>();
        for (int storage : storages) s.add(storage);
