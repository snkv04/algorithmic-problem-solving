...
public class PhoenixAndScience {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            long a = 1, b = 1;
            PriorityQueue<Long> bacteriaPerDay = new PriorityQueue();
            bacteriaPerDay.add((long)1);
            while (true) {
                b *= 2;
                if (a + b > n) {
                    if (a != n) {
                        bacteriaPerDay.add(n-a);
                        break;
                    } else {
                        break;
                    }
                    ...
                    a += b;
                    bacteriaPerDay.add(b);
                }
            }
            // pw.println(bacteriaPerDay);
            pw.println(bacteriaPerDay.size()-1);
            long last = bacteriaPerDay.poll();
            while (!bacteriaPerDay.isEmpty()) {
                pw.print(bacteriaPerDay.peek()-last + " ");
                last = bacteriaPerDay.poll();
            }
            pw.println();
        }
