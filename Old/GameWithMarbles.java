...
public class GameWithMarbles {
    ...
    static void solve() throws IOException {
        ...
        long[] b = Arrays.stream(br.readLine().split(" ")).mapToLong(Integer::parseInt).toArray();
        for (int i = 0; i <= n-1; i++) { a[i]--; b[i]--; }
        PriorityQueue<Color> marbles = new PriorityQueue<>(Comparator.comparingLong(o -> -(o.ai+o.bi)));
        for (int i = 0; i<= n-1; i++) {
            marbles.add(new Color(a[i], b[i], i));
        }
        // while (!aliceBest.isEmpty()) pw.println(aliceBest.poll());
        // pw.println("alice done, now bob");
        // while (!bobBest.isEmpty()) pw.println(bobBest.poll());
        // pw.println();
        long sum = 0;
        for (int i = 0; i <= n-1; i++) {
            if (i%2==0) {
                Color color = marbles.poll();
                sum += color.ai;
            } else {
                Color color = marbles.poll();
                sum -= color.bi;
    ...
    static class Color {
        ...
        public Color(long ai, long bi, long colorNum) {
            this.ai = ai;
            this.bi = bi;
        ...
        //     return "(ai = "+ai+", bi = "+bi+", colorID = "+colorNum+")";
        // }
    }

    // static class Comp1 implements Comparator<Color> {
    //     public int compare(Color c1, Color c2) {
    //         if (c1.ai != c2.ai) return -Long.compare(c1.ai, c2.ai);
    //         else return -Long.compare(c1.bi, c2.bi);
    //     }
    // }

    // static class Comp2 implements Comparator<Color> {
    //     public int compare(Color c1, Color c2) {
    //         if (c1.bi != c2.bi) return -Long.compare(c1.bi, c2.bi);
    //         else return -Long.compare(c1.ai, c2.ai);
    //     }
    // }
