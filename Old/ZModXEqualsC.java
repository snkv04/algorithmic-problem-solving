...
public class ZModXEqualsC {
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            // we know that a = x%y, so whatever y is, x should be y+ka for some integer k.
            // k could be 1, so x is simply equal to y+a. now, we also know that b = y % z,
            // so y can be z+b, making x equal to z + b + a. since z % x == c, then either
            // z is less than x, making z just c, or z is greater than x, making z = jc + x
            // for some integer j. however, earlier we made the assumption that x=z+a+b, so
            // to keep that noncontradictory, we can make z = c. now, none of the assumptions
            // are broken, and we have that x > y > z with x=a+b+c, y=b+c, and z=c.
            long x = a+b+c;
            long y = b+c;
            long z = c;
            pw.println(x+" "+y+" "+z);
