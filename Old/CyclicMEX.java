...
public class CyclicMEX {
    ...
    static void solve() throws IOException {
        ...
        for (int i = n-1; i >= 0; i--) {
            ...
            // TreeSet<Integer> excluded = new TreeSet<>(); for (int j = 0; j <= n-1; j++) excluded.add(j);
            // int sum = 0;
            // for (int j = 0; j <= n-1; j++) {
            //     int num = p[(i+j)%n];
            //     excluded.remove(num);
            //     pw.print(excluded.isEmpty()?n:excluded.first()+" + ");
            //     sum += excluded.isEmpty()?n:excluded.first();
            // }
            // pw.println(" = "+sum);
