...
public class TornLuckyTicket {
    ...
    public static void main(String[] args) throws IOException {
        ...
        char[][] tickets = new char[n][0];
        for (int i = 0; i <= n-1; i++) {
            tickets[i] = line[i].toCharArray();
        }

        Multiset[] count = new Multiset[6];
        for (int i = 1; i <= 5; i++) count[i] = new Multiset();
        for (int i =0; i <= n-1; i++) {
            int sum = 0;
            for (char c : tickets[i]) sum += (int) (c - '0');
            count[tickets[i].length].add(sum);
        }
        // for (int i = 1; i <= 5; i++) pw.println("i="+i+", and count[i] = "+count[i].mset);

        int start = -1;
        long ans = 0;
        for (int i= 0; i <= n-1; i++) {
            if (tickets[i].length%2==0) start = 2;
            else start = 1;
            // pw.println("current ticket = "+Arrays.toString(tickets[i])+", start="+start);

            for (int j = start; j < tickets[i].length; j+=2) {
                int half = (j+tickets[i].length)/2;
                int sumFirst = 0; 
                for (int k = 0; k < half; k++)
                    sumFirst += (int) (tickets[i][k]-'0');
                for (int k = half; k <= tickets[i].length-1; k++)
                    sumFirst -= (int) (tickets[i][k]-'0');
                int sumLast = 0;
                for (int k = tickets[i].length-1; k >= tickets[i].length-half; k--)
                    sumLast += (int) (tickets[i][k]-'0');
                for (int k = tickets[i].length-half-1; k >= 0; k--) 
                    sumLast -= (int) (tickets[i][k]-'0');
                // pw.println("j="+j+", so given the curren't ticket's length, half="+half);
                // pw.println("sumfirst = "+sumFirst+", so looking in count["+j+"] for sum of "+sumFirst);
                // pw.println("sumlast = "+sumLast+", so looking in count["+j+"] for sum of "+sumLast);
                
                if (count[j].mset.containsKey(sumFirst)) ans += count[j].mset.get(sumFirst);
                if (count[j].mset.containsKey(sumLast)) ans += count[j].mset.get(sumLast);
            }

            int sum = 0;
            for (int k = 0; k <= tickets[i].length-1; k++) {
                sum += (int) (tickets[i][k]-'0');
            }
            if (count[tickets[i].length].mset.containsKey(sum)) ans += count[tickets[i].length].mset.get(sum);
            // pw.println();
        }
        pw.println(ans);
