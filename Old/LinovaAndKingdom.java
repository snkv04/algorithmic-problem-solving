...
public class LinovaAndKingdom {
    ...
    public static void main(String[] args) throws IOException {
        ...
        adjLists = new ArrayList[n+1]; for (int i = 1; i <= n; i++) adjLists[i] = new ArrayList<>();
        for (int i = 1; i <= n-1; i++) {
        ...
        visited = new boolean[n+1];
        dependent = new int[n+1];
        dfs(1);
        // pw.println(Arrays.toString(dependent));

        PriorityQueue<Node> nodes = new PriorityQueue<Node>();
        for (int i = 1; i <= n; i++) nodes.add(new Node(i, dist[i], dependent[i]));
        long ans = 0;
        for (int i = 1; i <= k; i++) {
            Node node = nodes.poll();
            ans += node.dist - node.dependent;
        }
        pw.println(ans);
    ...
    static void dfs(int node) {
        ...
        for (int next : adjLists[node]) {
            if (!visited[next]) {
                dfs(next);
                numDependent += dependent[next]+1;
            }
        }
        dependent[node] = numDependent;
    }

    static class Node implements Comparable<Node> {
        int id, dist, dependent;

        public Node(int id, int dist, int dependent) {
            this.id = id;
            this.dist = dist;
        ...
        public int compareTo(Node other) {
            if (this.dist-this.dependent != other.dist-other.dependent)
                return -Integer.compare(this.dist-this.dependent, other.dist-other.dependent);
            else return Integer.compare(this.id, other.id);
        }
    }
