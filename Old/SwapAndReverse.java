...
public class SwapAndReverse {
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            if (k%2==0) {
                // can literally just bubble sort if k==2
                // can do a roundabout bubble sort if k is even but not 2
                ArrayList<Character> list = new ArrayList<>();
                for (char c : s) list.add(c);
                Collections.sort(list);
                for (char c : list) pw.print(c);
                pw.println();
            } else {
                ArrayList<Character> one = new ArrayList<>();
                ArrayList<Character> two = new ArrayList<>();
                for (int i = 0; i <= n-1; i++) {
                    if (i%2==0) one.add(s[i]);
                    else two.add(s[i]);
                }
                ...
                for (int i = 0; i <= one.size()-1; i++) {
                    pw.print(one.get(i));
                    if (i < two.size()) pw.print(two.get(i));
                }
                pw.println();
            }
        }
