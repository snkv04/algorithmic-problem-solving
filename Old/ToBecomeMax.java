...
public class ToBecomeMax {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            if (n==1) { pw.println(a[0]); continue; }
            int max = Integer.MIN_VALUE; for (int num : a) max = Math.max(max, num);
            long l = 0, r = (int)1e9, ans = -1;
            while (l <= r) {
                int mid = (int) ((l+r) / 2);
                if (check(mid)) {
                    l = mid+1;
    ...
    static boolean check(int x) {
        ...
        for (int i = 0; i <= n-2; i++) {
            ...
            if (j==n-1 && a[n-1]>=b[j-1]-1) return true;
        }
        return false;
    }
