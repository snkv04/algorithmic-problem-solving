        ...
        while (q-->0) {
            ...
            for (int i = 0; i <= n; i++) {
                for (int j = 1; j <= 2*n-1; j++) {
                    memo[i][j] = memo[i][j-1];
                    if (i != 0) memo[i][j] = Math.min(memo[i][j], memo[i-1][j-1] + Math.abs(t[i-1] - j));
                    // pw.println();
                }
            }
            pw.println(memo[n][2*n-1]);
            for (int i = 0; i <= n; i++) pw.println("i="+i+", and array="+Arrays.toString(memo[i]));
        }
