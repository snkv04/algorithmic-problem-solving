...
public class YetnotherrokenKeoard {
    ...
    static void solve() throws IOException {
        ...
        boolean[] keep = new boolean[n]; Arrays.fill(keep, true);
        for (int i = 0; i <= n-1; i++) {
            if (s[i]=='b') {
                keep[i] = false;
                if (!lower.isEmpty()) keep[lower.pop()] = false;
            } else if (s[i]=='B') {
                keep[i] = false;
                if (!upper.isEmpty()) keep[upper.pop()] = false;
            } else {
                if ((int)s[i] <= 90) {
                    upper.add(i);
                } else {
        ...
        for (int i = 0; i <= n-1; i++) {
            if (keep[i]) result.add(s[i]);
        }
        for (char c : result) pw.print(c);
        pw.println();
    }
