    ...
    public static void main(String[] args) throws IOException {
        ...
        while (tc-->0) {
            ...
            if (sum % n != 0) {
                ...
                long avg = sum / n;
                PriorityQueue<Long> give = new PriorityQueue<>();
                PriorityQueue<Long> take = new PriorityQueue<>();
                boolean works = true;
                int change0s = 0;
                for (int i = 0; i <= n-1 && works; i++) {
                    long change = (long)Math.abs(avg - a[i]);
                    if (change == 0) { change0s++; continue; }
                    long incexp = -1;
                    for (long j = 0; j <= 35; j++) {
                        if ((((long)1)<<j) > change) {
                            incexp = (int)j;
                    ...
                    long diff = (((long)1)<<incexp) - change;
                    long decexp = -1;
                    for (long k = 0; k <= 35; k++) {
                        if ((((long)1)<<k) > diff) {
                            decexp = (int)k-1;
                    ...
                    // pw.println("i = "+(i+1)+", incexp and decexp are "+(1<<incexp)+" "+(1<<decexp));
                    // pw.println("also change is "+change);
                    if ((((long)1 << incexp) - ((long)1<<decexp)) != Math.abs(change)) works = false;
                    else {
                        if (avg-a[i] > 0) {
                            take.add((long)1<<incexp);
                            give.add((long)1<<decexp);
                        } else {
                            take.add((long)1<<decexp);
                            give.add((long)1<<incexp);
                        }
                    }
                }

                // pw.println(give); pw.println(take); pw.println(change0s);
                while (!give.isEmpty() && works) {
                    if (give.peek() == take.peek()) {
                        give.poll(); take.poll();
                    } else {
                        if (change0s == 0) {
                            works = false;
                        } else {
                            long takefirst = take.poll();
                            long givefirst = give.poll();
                            take.add(givefirst);
                            give.add(takefirst);
                            change0s--;
                        }
