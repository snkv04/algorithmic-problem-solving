...
public class GrayCode {
    ...
    static void solve() throws IOException {
        ...
        char[] a = new char[n]; Arrays.fill(a, '0'); answers.add(a);
        char[] b = new char[n]; Arrays.fill(b, '0'); b[n-1] = '1'; answers.add(b);
        for (int i = n-2; i >= 0; i--) {
            int currLen = answers.size();
            for (int j = currLen-1; j >= 0; j--) {
                char[] bits = Arrays.copyOf(answers.get(j), n);
                bits[i] = '1';
                answers.add(bits);
            }
        }
