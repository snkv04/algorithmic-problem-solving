...
public class OceanMonument {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            long[][] memo = new long[n+e+1][e+1];
            for (int i = 0; i <= n+e; i++) memo[i][0] = 1;
            for (int i = 1; i <= e; i++) {
                for (int j = 0; j <= n+e; j++) {
                    //
                }
            ...
            // memo = new long[n + e + 1][e+1];
            // for (int i = 0; i <= n+e; i++) Arrays.fill(memo[i], -1);
    ...
    // static long solve(int normal, int elder) {
    //     // System.out.println("calling solve("+normal+", "+elder+")");
    //     if (memo[normal][elder] != -1) return memo[normal][elder];
    //     if (normal <= 1) {
    //         if (elder == 0) { memo[normal][elder] = 1; return 1; }
    //         else return solve(normal+1, elder-1);
    //     }

    //     long ans = 0;
    //     ans += solve(normal-1, elder);
    //     if (elder > 0) ans += solve(normal+1, elder-1);
    //     memo[normal][elder] = ans;
