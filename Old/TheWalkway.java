...
public class TheWalkway {
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            double d = Integer.parseInt(line[2]);
            int[] a = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
            long count = 1;
            int last = 1;
            for (int i = 0; i <= m-1; i++) {
                count += (int)Math.round(Math.ceil((double)(a[i]-last) / (d)));
                last = a[i];
            }
            count += (int)Math.round(Math.floor((double)(n-a[m-1])/(d)));

            long count2 = 0, count3 = 0;
            last = 1;
            for (int i = 0; i <= m-2; i++) {
                double with = (int)Math.round(Math.ceil((double)(a[i]-last)/d)) + (int)Math.round(Math.ceil((double)(a[i+1]-a[i])/d));
                double without = (int)Math.round(Math.ceil((double)(a[i+1]-last)/(d)));
                if (with - without == 1) count2++;
                if (with - without == 2) count3++;
                last = a[i];
            }
            int without = (int)Math.round(Math.floor((double)(n-a[m-2])/d));
            int with = (int)Math.round(Math.ceil((double)(a[m-1]-a[m-2])/(d))) + (int)Math.round(Math.floor((double)(n-a[m-1])/d));
            if (with > without) count2++;

            if (count2 > 0) pw.println((count-1)+" "+count2);
            else pw.println(count+" "+m);
        }
