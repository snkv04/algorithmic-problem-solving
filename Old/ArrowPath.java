...
public class ArrowPath {
    ...
    static void solve() throws IOException {
        ...
        adj = new ArrayList[2*n]; for (int i =0 ; i <= 2*n-1; i++) adj[i] = new ArrayList<>();
        for (int i = 0; i <= 2*n-1; i++) {
            int r = i%2, c = i/2;
            if ((r+c)%2 == 0) {
                if (i<2*n-2) adj[i].add(i+2);
                if (i >= 2) adj[i].add(i-2);
                if (r==1) adj[i].add(i-1);
                else adj[i].add(i+1);
            } else {
                if (grid[r][c] == '<') {
                    if (c >= 1) adj[i].add(i-2);
                } else {
                    if (c <= n-2) adj[i].add(i+2);
                }
            }
        }
        ...
        dfs(0);
        pw.println(visited[2*n-1] ? "YES" : "NO");
    }
