...
public class MishkaAndTheLastExam {
    ...
    static void solve() throws IOException {
        ...
        Queue<Long> start = new LinkedList<>();
        Stack<Long> end = new Stack<>();
        long max = 0, min = (long)1e18;
        for (long num : b) {
            long l = max, r = min, lower = -1;
            while (l <= r) {
                long mid = (l+r)/2;
                if ((num-mid) <= min) {
                    lower = mid;
            ...
            min = Math.min(min, num-lower);
            start.add(lower);
            end.add(num-lower);
        }

        while(!start.isEmpty())pw.print(start.poll()+" ");
        while(!end.isEmpty())pw.print(end.pop()+" ");
        pw.println();
    }
