...
public class BeautifulArray {
    ...
    static void solve() throws IOException {
        ...
        int n = Integer.parseInt(asdf[0]), x = Integer.parseInt(asdf[1]);
        long[] a = new long[n];
        ...
        long[] ka = new long[n], kar = new long[n], kb = new long[n], kbr = new long[n];
        for (int i = 0; i <= n-1; i++) {
            ka[i] = Math.max(a[i], a[i] + (i==0 ? 0 : ka[i-1]));
            kb[i] = Math.max(b[i], b[i] + (i==0 ? 0 : kb[i-1]));
        }
        for (int i = n-1; i >= 0; i--) {
            kar[i] = Math.max(a[i], a[i] + (i==n-1 ? 0 : kar[i+1]));
            kbr[i] = Math.max(b[i], b[i] + (i==n-1 ? 0 : kbr[i+1]));
        }
        // pw.println(Arrays.toString(ka));
        // pw.println(Arrays.toString(kb));
        // pw.println(Arrays.toString(kar));
        // pw.println(Arrays.toString(kbr));

        long ans = 0;
        for (int i = -1; i <= n-1; i++) {
            long lefta = Math.max(0, i==-1 ? 0 : ka[i]);
            long leftb = Math.max(0, i==-1 ? 0 : kb[i]);
            long righta = Math.max(0, i==n-1 ? 0 : kar[i+1]);
            long rightb = Math.max(0, i==n-1 ? 0 : kbr[i+1]);
            ans = Math.max(ans, Math.max(Math.max(lefta+righta, lefta+rightb), Math.max(leftb+righta, leftb+rightb)));
        }
        pw.println(ans);
    }
