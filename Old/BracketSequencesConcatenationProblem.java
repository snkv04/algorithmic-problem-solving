...
public class BracketSequencesConcatenationProblem {
    public static void main(String[] args) throws IOException {
        ...
        Multiset balances = new Multiset();
        long ans = 0;
        while (n-->0) {
            int balance = 0;
            ...
            for (int i = 0; i <= len-1; i++) {
                balance += s[i]=='('?1:-1;
                if (balance < 0) { start = false; break; }
            }
            balance = 0;
            for (int i = len-1; i >= 0; i--) {
                balance += s[i]=='('?1:-1;
                if (balance > 0) { end = false; break; }
            }
            if (start || end) {
                balance = 0;
                for (int i = len-1; i >= 0; i--) {
                ...
                if (balance == 0) {
                    ans += 1;
                    if (balances.mset.get(0)!=null) ans += 2*balances.mset.get(0);
                } else {
                    if (balances.mset.get(-balance)!=null) ans += balances.mset.get(0-balance);
                }
