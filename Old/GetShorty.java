...
public class GetShorty {
    ...
    static void solve() throws IOException {
        ...
        int n = Integer.parseInt(il[0]), m = Integer.parseInt(il[1]);
        while (n > 0 || m > 0) {
            adj = new ArrayList[n];
            for (int i = 0; i <= n-1; i++) adj[i] = new ArrayList<>();
            for (int i = 1; i <= m; i++) {
                il = br.readLine().split(" ");
                int a = Integer.parseInt(il[0]);
                int b = Integer.parseInt(il[1]);
                double f = Double.parseDouble(il[2]);
                adj[a].add(new Edge(b, f));
                adj[b].add(new Edge(a, f));
            }

            dist = new double[n];
            dist[0] = 1;
            PriorityQueue<Pair> pq = new PriorityQueue<>(Comparator.comparingDouble(o -> -o.b));
            pq.add(new Pair(0, 1));
            while (!pq.isEmpty()) {
                Pair node = pq.poll();
                if (dist[node.a] > node.b) continue;
                for (Edge next : adj[node.a]) {
                    // pw.println("currenly considering edge from "+node+" to "+next.to+", with frac = "+next.weight);
                    if (dist[node.a] * next.weight > dist[next.to]) {
                        dist[next.to] = dist[node.a] * next.weight;
                        pq.add(new Pair(next.to, dist[next.to]));
                    }
                }
            }
            pw.println(String.format("%.4f", dist[n-1]));

            il = br.readLine().split(" ");
            n = Integer.parseInt(il[0]); m = Integer.parseInt(il[1]);
        }
    }
