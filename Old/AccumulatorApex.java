...
public class AccumulatorApex {
    ...
    static void solve() throws IOException {
        ...
        Queue<Segment>[] segments = new Queue[k];
        for (int i = 0; i <= k-1; i++) {
            ...
            for (int j = 1; j <= a[0]; j++) {
                if (sum > 0 && a[j]<0) {
                    segments[i].add(new Segment(min, sum));
                    sum = 0;
                    min = 0;
                }
                sum += a[j];
        ...
        // for (int i = 0 ;i <= k-1; i++) pw.println(segments[i]);

        PriorityQueue<Integer> pq = new PriorityQueue<>(Comparator.comparingLong(o -> -segments[o].peek().barrier));
        for (int i = 0; i <= k-1; i++) {
            if (segments[i].size() > 0) pq.add(i);
        }
        while (true) {
            if (pq.size() == 0 || Math.abs(segments[pq.peek()].peek().barrier) > x) break;

            int index = pq.poll();
            x += segments[index].poll().value;
            if (!segments[index].isEmpty()) pq.add(index);
        }
        pw.println(x);
    }
    ...
    static class Segment {
        ...
        public Segment(long barrier, long value) {
            this.barrier = barrier;
            this.value = value;
        }

        public String toString() {
            return "(barrier = "+barrier+", value = "+value+")";
        }
    }
