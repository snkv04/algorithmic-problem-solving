...
public class SearchInParallel {
    ...
    static void solve() throws IOException {
        ...
        long s1 = Integer.parseInt(line[1]), s2 = Integer.parseInt(line[2]);
        int[]r = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        PriorityQueue<Pair> pairs = new PriorityQueue<>(Comparator.comparingInt(o -> -o.times));
        for (int i = 1; i <= n; i++) {
            pairs.add(new Pair(i, r[i-1]));
        }
