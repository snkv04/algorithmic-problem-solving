...
public class Bicycles {
    ...
    static void solve() throws IOException {
        ...
        HashSet<Integer> slows = new HashSet<>();
        ArrayList<Pair>[] adj = new ArrayList[n];
        for (int i = 0; i <= n-1; i++) adj[i] = new ArrayList<>();
        for (int i = 0; i <= m-1; i++) {
            inp = br.readLine().split(" ");
            int u = Integer.parseInt(inp[0])-1, v = Integer.parseInt(inp[1])-1, w = Integer.parseInt(inp[2]);
            adj[u].add(new Pair(v, w));
            adj[v].add(new Pair(u, w));
        }
        int[] s = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        for (int num : s) slows.add(num);
        Map<Pair, Long> dist = new HashMap<>();
        for (int i = 0; i <= n-1; i++) for (int slowness : slows) dist.put(new Pair(i, slowness), Long.MAX_VALUE);
        dist.put(new Pair(0, s[0]), (long)0);
        // System.out.println("dist map = ");
        // for (Map.Entry<Pair, Long> entry : dist.entrySet()) {
        //     System.out.println("\t(city="+entry.getKey().a+", slowness="+entry.getKey().b+", min dist="+entry.getValue()+")");
        // }

        PriorityQueue<Pair> pq = new PriorityQueue<>(Comparator.comparingLong(o -> dist.get(o)));
        pq.add(new Pair(0, s[0]));
        while (!pq.isEmpty()) {
            // node has index and s[i] value
            // edge has "to" and "weight"
            Pair node = pq.poll();
            // System.out.println("city="+node.a+", slowness="+node.b);
            // System.out.println(dist.containsKey(node));
            // System.out.println(dist.containsKey(new Pair(node.a, node.b)));
            for (Pair edge : adj[node.a]) {
                // System.out.println("\tedge goign to "+edge.a+" with length "+edge.b);
                if (dist.get(node) + node.b * edge.b
                    < dist.get(new Pair(edge.a, Math.min(node.b, s[edge.a])))) {

                    dist.put(new Pair(edge.a, Math.min(node.b, s[edge.a])), dist.get(node) + node.b * edge.b);
                    pq.add(new Pair(edge.a, Math.min(node.b, s[edge.a])));
                }
            }
        }

        long ans = Long.MAX_VALUE;
        for (int slowness : slows) ans = Math.min(ans, dist.get(new Pair(n-1, slowness)));
        pw.println(ans);
    }
    ...
    static class Comp implements Comparator<Pair> {
        public int compare(Pair p1, Pair p2) {
            ...
            else return Integer.compare(p1.b, p2.b);
        }
    }
