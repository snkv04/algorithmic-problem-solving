...
public class Potions {
    public static void main(String[] args) throws IOException {
        ...
        // long[] memo = new long[n+1]; // memo[j] represents the max health obtainable by drinking j potions
        // Arrays.fill(memo, Long.MIN_VALUE / 2);
        // memo[0] = 0;
        // for (int i = 1; i <= n; i++) {
        //     for (int j = i; j >= 1; j--) {
        //         if (memo[j-1] + a[i-1] >= 0) memo[j] = Math.max(memo[j], memo[j-1] + a[i-1]);
        //     }
        // }
        ...
        long sum = 0;
        for (int i = 0; i <= n-1; i++) {
            // basically, if you can use it, then you should since that optimizes the number
            // of potions you drink, but if you can't, then see if you can improve the current
            // state of the potions you've drank so far for the sake of future steps
            if (sum + a[i] >= 0) {
                sum += a[i];
                pq.add(a[i]);
            } else {
