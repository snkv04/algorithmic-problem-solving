...
public class LargestSubsequence {
    ...
    static void solve() throws IOException {
        ...
        char[] largest = new char[n];
        largest[n-1] = s[n-1];
        for (int i = n-2; i >= 0; i--) {
            largest[i] = (char) Math.max(largest[i+1], s[i]);
        }
        // pw.println(Arrays.toString(largest));

        Queue<Character> notIn = new LinkedList<>();
        ...
        int ans = 0;
        Queue<Character> newIn = new LinkedList<>();
        while (in.peek() != largest[0]) {
            ans++;
            newIn.add(in.pop());
        }
        while (!in.isEmpty()) newIn.add(in.pop());

        char[] after = new char[n];
        for (int i = 0; i <= n-1; i++) after[i] = is[i]?newIn.poll():notIn.poll();
        // pw.println(sorted);
        // pw.println(after);

        boolean works = true;
        for (int i = 0; i <= sorted.size()-1 && works; i++) {
            if (sorted.get(i)!=after[i]) works = false;
        }
