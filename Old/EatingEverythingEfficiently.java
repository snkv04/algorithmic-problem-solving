...
public class EatingEverythingEfficiently {
    static int MOD = (int) (1e9 + 7); // 998244353;
    static ArrayList<Integer>[] adjLists;
    static Queue<Integer> topological;
    ...
    public static void main(String[] args) throws IOException {
        ...
        long[] c = Arrays.stream(br.readLine().split(" ")).mapToLong(Integer::parseInt).toArray();
        ...
        // pw.println(topological);

        double[] max = new double[n];
        while (!topological.isEmpty()) {
            int node = topological.poll();
            double maxAdj = 0;
            for (int adj : adjLists[node]) {
                maxAdj = Math.max(maxAdj, max[adj]);
            }
            max[node] = Math.max(maxAdj, (double) c[node] + maxAdj/2.0);
            // pw.println("looking at "+node+", max is currently "+Arrays.toString(max));
        }

        double maxSatisfaction = 0;
        for (int i = 0; i <= n-1; i++) maxSatisfaction = Math.max(maxSatisfaction, max[i]);
        pw.println(maxSatisfaction);
    ...
    static void dfs(int node) {
        ...
        visited[node] = true;
        for (int next : adjLists[node]) dfs(next);
