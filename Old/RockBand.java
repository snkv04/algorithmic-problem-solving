...
public class RockBand {
    ...
    static void solve() throws IOException {
        /*
         * given a matrix, we want to find the leftmost column such that the matrix with its right side
         * on that column has all of its rows composed of the same elements
         */


        String[] tokens = br.readLine().split(" "); // still don't know what to call this
        int m = Integer.parseInt(tokens[0]), s = Integer.parseInt(tokens[1]);
        int[][] a = new int[m][s]; for (int i = 0; i < m; i++) a[i] = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();

        ////////

        // HashMap<Integer, Integer>[] map = new HashMap[m]; for (int i = 1; i < m; i++) map[i] = new HashMap<>();
        // for (int i = 1; i < m; i++) {
        //     for (int j = 0; j < s; j++) {
        //         map[i].put(a[i][j]-1, j);
        //     }
        // }
        // // for (int i =0 ; i < m; i++) System.out.println("map["+i+"] = "+map[i]);

        // int ans = 0;
        // HashMap<Integer, Integer> maxIdxSeen = new HashMap<>(); for (int i = 1; i < m; i++) maxIdxSeen.put(i, -1);
        // for (int i = 0; i < s; i++) {
        //     int song = a[0][i]-1;
        //     // System.out.println("i = "+i+", song = "+song);
        //     TreeSet<Integer> maxIndices = new TreeSet<>();
        //     for (int j = 1; j < m; j++) {
        //         // System.out.println("j="+j+", song="+song);
        //         maxIdxSeen.put(j, Math.max(
        //             maxIdxSeen.get(j),
        //             map[j].get(song)
        //         ));
        //         maxIndices.add(maxIdxSeen.get(j));
        //     }
        //     if (maxIndices.size()==1 && maxIndices.first() == i) {
        //         ans = i + 1;
        //         break;
        //     }
        // }
        ...
        Multiset songs2 = new Multiset(), freqs = new Multiset();
        int ans = 0;
        for (int j = 0; j < s; j++) {
            for (int i = 0; i < m; i++) {
                int song = a[i][j];
                if (songs2.count(song) > 0) freqs.remove(songs2.count(song));
                songs2.add(song);
                freqs.add(songs2.count(song));
            ...
            if (freqs.mset.size() == 1 && freqs.mset.firstKey() == m) {
                ans = j+1;
                break;
            }
        }
        ...
        for (int song : songs) pw.print(song+" ");
        pw.println();
    }
