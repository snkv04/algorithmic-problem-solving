...
public class PickUpSticks {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int[] indexInOrder = new int[n];
        for (int i = 0; i <= n-1; i++) {
            int node = reverse.pop();
            order.add(node);
            indexInOrder[node] = i;
        }
        ...
        for (int i = 0; i <= n-1 && works; i++) {
            for (int next : adjLists[i]) {
                if (indexInOrder[i] > indexInOrder[next]) {
                    works = false; break;
                }
            }
        }
        if (!works) pw.print("IMPOSSIBLE");
        else for (int node : order) pw.println(node+1);
