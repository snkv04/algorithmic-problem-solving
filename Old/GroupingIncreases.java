...
public class GroupingIncreases {
    ...
    static void solve() throws IOException {
        ...
        if (n <= 2) { pw.println(0); return; }

        Stack<Integer> s1 = new Stack<>(), s2 = new Stack<>();
        s1.add(Integer.MAX_VALUE); s2.add(Integer.MAX_VALUE);
        int ans = 0;
        int i = 0;
        for (i = 0; i <= n-1; i++) {
            int num = a[i];
            if (num > s1.peek() && num > s2.peek()) {
                ans++;
                if (s1.peek() <= s2.peek()) s1.add(num);
                else s2.add(num);
            } else if (num <= s1.peek() && num <= s2.peek()) {
                if (s1.peek() <= s2.peek()) s1.add(num);
                else s2.add(num);
            } else if (num > s1.peek()) {
                s2.add(num);
