...
public class NoTreesButFlowers {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int n = Integer.parseInt(line[1]); while (n-->0) {
            double a, b, h;
            line = br.readLine().split(" ");
            a = Double.parseDouble(line[0]);
            b = Double.parseDouble(line[1]);
            h = Double.parseDouble(line[2]);
            list.add(integral(a, b, h));
        }

        double min = Integer.MAX_VALUE;
        int ans = -1;
        int i = 0;
        for (double volume : list) {
            // pw.println(volume);
            if (Math.abs(volume - v) < min) {
                min = Math.abs(v - volume);
                ans = i;
            }
            i++;
        }
    ...
    static double integral(double a, double b, double h) {
        double deltaX = h / 100000.0, sum = 0;
        for (int i = 1; i <= 100000; i++) {
            double start = (i-1)*deltaX;
            double end = i*deltaX;
            double trapezoidArea = deltaX * (Math.pow(f(start, a, b), 2) + Math.pow(f(end, a, b), 2)) / 2;
            sum += trapezoidArea;
        }
        return sum * Math.PI;
    }
    ...
    static double f(double x, double a, double b) {
        ...
        return a * Math.pow(Math.E, exp) + b * Math.sqrt(x);
    }
