...
public class VeryDifferentArray {
    ...
    static void solve() throws IOException {
        ...
        Integer[] a = new Integer[n]; for (int i = 0; i <= n-1; i++) a[i] = Integer.parseInt(asdf[i]);
        Arrays.sort(a);
        asdf = br.readLine().split(" ");
        Integer[] b = new Integer[m]; for (int i = 0; i <= m-1; i++) b[i] = Integer.parseInt(asdf[i]);
        Arrays.sort(b);
        // pw.println("a = " + Arrays.toString(a));
        // pw.println("b = " + Arrays.toString(b));

        long ans = 0;
        int i1 = 0, j1 = n-1, i2 = 0, j2 = m-1;
        while (i1 <= j1) {
            if (Math.abs(a[i1]-b[j2]) >= Math.abs(a[j1]-b[i2])) {
                ans += Math.abs(a[i1]-b[j2]);
                i1++;
                j2--;
            } else {
                ans += Math.abs(a[j1]-b[i2]);
                i2++;
                j1--;
            }
        }
