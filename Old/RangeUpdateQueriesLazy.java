 ...
 * @author Sri Vangaru
 */
public class RangeUpdateQueriesLazy {
    ...
    static void solve() throws IOException {
        ...
        while (q-->0) {
            ...
            if (split[0].charAt(0) == '1') {
                int a = Integer.parseInt(split[1])-1, b = Integer.parseInt(split[2])-1, u = Integer.parseInt(split[3]);
                lz.update(a, b, 2, u);
            } else {
    ...
    static class Lazy {
        ...
        public long get(int idx, int v, int vl, int vr) {
            if (vl == vr) return t[v];
            pushDown(v, vl, vr);
            int vm = (vl + vr) / 2;
            if (idx <= vm) {
                return get(idx, 2*v, vl, vm);
            } else {
                return get(idx, 2*v+1, vm+1, vr);
            }
        }
    }
