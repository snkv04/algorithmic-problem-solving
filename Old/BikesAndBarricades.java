...
public class BikesAndBarricades {
    ...
    static void solve() throws IOException {
        ...
        double min = Integer.MAX_VALUE / 2;
        while (n-->0) {
            int[] a = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
            int x1 = a[0], y1 = a[1], x2 = a[2], y2 = a[3];
            if ((x1 < 0 && x2 < 0) || (x1 > 0 && x2 > 0)) continue;
            Point[] points = new Point[2];
            points[0] = new Point(x1, y1); points[1] = new Point(x2, y2);
            Arrays.sort(points, Comparator.comparingInt(o -> o.x));
            double slope = (double) ((double) points[1].y-points[0].y) / (double) (points[1].x - points[0].x);
            double dx = 0 - points[0].x;
            double yint = points[0].y + (slope * dx);
            if (yint > 0) min = Math.min(min, yint);
        }
