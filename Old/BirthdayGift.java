...
public class BirthdayGift {
    ...
    static void solve() throws IOException {
        ...
        int xor = 0;
        for (int num : a) xor = xor ^ num;
        for (int i = 30; i >= 0; i--) {
            int b1 = (1 << i) & xor, b2 = (1 << i) & x;
            if (b1 != b2) {
                if (b1 > b2) {
                    pw.println(-1);
                    return;
        ...
        ArrayList<Integer> last = new ArrayList<>(), curr;
        for (int num : a) last.add(num);
        pw.println(last);
        for (int i = 4; i >= 0; i--) {
            curr = new ArrayList<>();
            int sum = 0;
            ...
            for (int num : last) {
                sum = sum ^ num;
                max = Math.max(max, (1<<i)&sum);
                if (((1<<i)&sum) <= ((1<<i)&x)) {
                    curr.add(sum);
                    sum = 0;
                }
            }
            pw.println("i="+i+", curr="+curr);
            if (max == 0 && ((1<<i)&x) == 1) {
                break;
            }
            last = curr;
        }
        pw.println(last.size());
    }
