...
public class CoveredPointsCount {
    ...
    static void solve() throws IOException {
        ...
        Multiset startOn = new Multiset();
        Multiset endOn = new Multiset();
        for (int i = 1; i <= n; i++) {
        ...
        for (Map.Entry<Long, Long> entry : startOn.mset.entrySet()) {
            points.add(entry.getKey());
        }
        for (long point : endOn.mset.keySet()) {
            points.add(point);
        }

        long last = points.first()-1;
        int covering = 0;
        Multiset covered = new Multiset();
        for (long point : points) {
            long dist = point - last - 1;
            covered.add(covering, dist);
            if (startOn.mset.containsKey(point)) {
                covering += startOn.mset.get(point);
            }
            covered.add(covering, 1);
            if (endOn.mset.containsKey(point)) {
                covering -= endOn.mset.get(point);
            }
            last = point;
        }
        for (long i = 1; i <= n; i++) {
            if (covered.mset.containsKey(i)) {
                pw.print(covered.mset.get(i)+" ");
            } else pw.print("0 ");
        }
    }
    ...
    static class Multiset { // ordered multiset
        TreeMap<Long, Long> mset = new TreeMap<>();
