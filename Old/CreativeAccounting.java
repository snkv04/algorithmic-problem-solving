...
public class CreativeAccounting {
    ...
    static void solve() throws IOException {
        ...
        int n = Integer.parseInt(inp[0]), l = Integer.parseInt(inp[1]), h = Integer.parseInt(inp[2]);
        int[] a = new int[n]; for (int i = 0; i < n; i++) a[i] = Integer.parseInt(br.readLine());
        int[] prefsums = new int[n+1]; for (int i = 1; i <= n; i++) prefsums[i] = prefsums[i-1] + a[i-1];

        int min = Integer.MAX_VALUE, max = 0;
        for (int length = l; length <= h; length++) {
            for (int start = length; start >= 1; start--) {
                int count = 0;
                if (prefsums[start] > 0) count++;
                for (int end = start+length; end < n+length; end += length) {
                    if (prefsums[Math.min(end, n)] - prefsums[end-length] > 0) count++;
                }
