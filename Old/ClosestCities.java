...
public class ClosestCities {
    ...
    static void solve() throws IOException {
        ...
        long[] right = new long[n], left = new long[n];
        for (int i = 1; i <= n-1; i++) {
            if (closest[i-1]==i) {
                right[i] = right[i-1] + 1;
            } else {
        ...
        while (m-->0) {
            ...
            int from = Integer.parseInt(asdf[0]), to = Integer.parseInt(asdf[1]);
            from--; to--;
            if (from < to) pw.println(right[to]-right[from]);
            else pw.println(left[to]-left[from]);
        }
    }
