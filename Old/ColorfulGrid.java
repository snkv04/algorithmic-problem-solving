...
public class ColorfulGrid {
    ...
    static void solve() throws IOException {
        ...
        if (k >= n+m-2 && k%2==(n+m)%2) {
            ...
            for (int i = 0; i <= n-1; i++) {
                Arrays.fill(hori[i], 'B');
                if (i != n-1) Arrays.fill(vert[i], 'B');
            }
            ...
            if (red) vert[n-2][m-1] = 'R'; red = !red;
            if (red) hori[n-2][m-2] = 'R'; red = !red;
            if (red) vert[n-2][m-2] = 'R';
            ...
            for (int i = 0; i <= n-1; i++) {
                for (int j = 0; j <= m-2; j++) pw.print(hori[i][j] + " ");
                pw.println();
            }
            for (int i = 0; i <= n-2; i++) {
                for (int j = 0; j <= m-1; j++) pw.print(vert[i][j] + " ");
                pw.println();
            }
        } else {
            pw.println("NO");
