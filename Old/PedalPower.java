...
public class PedalPower {
    ...
    static void solve() throws IOException {
        /*
         * what do we know?
         * - there are bike edges that take certain amounts of time
         * - there are non-bike edges that take certain amounts of time
         * - if we go on a non-bike edge, we have to leave the bike at the beginning,
         *      and if we want to use the bike again, then we have to pick it up
         * - if only looking at the bike edges (or non-bike edges), this is a simple graph
         * - keep in mind that edges are bidirectional (might be useful)
         * - what we want to do is start from 0, go through all the vertices in the given order
         *      (although we can visit the same vertex multiple times), and then return to 0,
         *      all in the minimum time
         * - we know that this is possible at least to do in some amount of time
         * - there may be some vertices that aren't required to visit
         */
