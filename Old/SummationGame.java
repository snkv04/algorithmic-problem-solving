...
public class SummationGame {
    ...
    static void solve() throws IOException {
        ...
        int n = Integer.parseInt(asdf[0]), k = Integer.parseInt(asdf[1]), x = Integer.parseInt(asdf[2]);
        int[] _a = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        Integer[] a = new Integer[n]; for (int i = 0; i <= n-1; i++) a[i] = _a[i];
        Arrays.sort(a);
        ...
        for (int i = n; i >= n-k; i--) {
            ...
            ans = Math.max(ans, pref[start] + -1*(pref[i]-pref[start]));
        }
        pw.println(ans);
    }
