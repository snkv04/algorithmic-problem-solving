...
public class ThreeActivities {
    ...
    static void solve() throws IOException {
        ...
        PriorityQueue<Integer> pqa = new PriorityQueue<>(Comparator.comparingInt(o -> -a[o]));
        PriorityQueue<Integer> pqb = new PriorityQueue<>(Comparator.comparingInt(o -> -b[o]));
        PriorityQueue<Integer> pqc = new PriorityQueue<>(Comparator.comparingInt(o -> -c[o]));
        for (int i = 0; i <= n-1; i++) {
            pqa.add(i); pqb.add(i); pqc.add(i);
        }
        ...
        for (int i = 0; i <= 2; i++) {
            for (int j = 0; j <= 2; j++) {
                for (int k = 0; k <= 2; k++) {
                    int day1 = days[0][i], day2 = days[1][j], day3 = days[2][k];
                    if (day1 != day2 && day1 != day3 && day2 != day3)
                        ans = Math.max(ans, a[day1]+b[day2]+c[day3]);
                }
            }
        }
        pw.println(ans);
