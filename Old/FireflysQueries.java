...
public class FireflysQueries {
    ...
    static void solve() throws IOException {
        ...
        pref[0] = a[0]; for (int i = 1; i <= n-1; i++) pref[i] = pref[i-1] + a[i];
        suf[n-1] = a[n-1]; for (int i = n-2; i >= 0; i--) suf[i] = suf[i+1] + a[i];
        while (q-->0) {
            split = br.readLine().split(" ");
            long l = Long.parseLong(split[0]), r = Long.parseLong(split[1]);
            l--; r--;
            long ans = 0;
            long li = l/n, lj = l%n, ri = r/n, rj=r%n;
            if (ri-li > 1) {
                ans += (long) (ri-li-1) * pref[n-1];
            }
            // System.out.println("l="+l+", li="+li+", lj="+lj+",   r="+r+", ri="+ri+", rj="+rj);

            ArrayList<Pair> toProcess = new ArrayList<>();
            if (ri == li) {
                toProcess.add(new Pair((li+lj)%n, (ri+rj)%n));
            } else {
                toProcess.add(new Pair((li+lj)%n, (li+n-1)%n));
                toProcess.add(new Pair(ri, (ri+rj)%n));
            }
            
            for (Pair bounds : toProcess) {
                l = bounds.a; r = bounds.b;
                if (l > r) {
                    ans += suf[(int)l] + pref[(int)r];
                } else {
                    ans += pref[(int)r];
                    if (l > 0) ans -= pref[(int)(l-1)];
                }
            }
            // pw.println("ans = "+ans);
            pw.println(ans);
