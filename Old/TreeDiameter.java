...
public class TreeDiameter {
    ...
    static void solve() throws IOException {
        ...
        // memo[i][0] is the longest path that goes through node i and has its ends in
        // two of node i's subtrees, and memo[i][1] is the longest path that starts at
        // node i and ends in one of its subtrees
        ...
        pw.println(max-1); // to count edges, not vertices
    }
    ...
    static void dfs(int node) {
        ...
        if (pq.size() == 0) {
            ...
            memo[node][0] = memo[node][1] = 1 + pq.peek();
        } else {
            int longest = pq.poll();
            memo[node][1] = longest + 1;
            int secondLongest = pq.poll();
            memo[node][0] = longest + secondLongest + 1;
        }
    }
