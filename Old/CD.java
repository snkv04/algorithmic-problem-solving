...
public class CD {
    ...
    static void solve() throws IOException {
        ...
        while (n > 0 || m > 0) {
            TreeSet<Integer> jack = new TreeSet<>(), jill = new TreeSet<>();
            while (n-- > 0) {
                jack.add(Integer.parseInt(br.readLine()));
            }
            while (m-- > 0) {
                jill.add(Integer.parseInt(br.readLine()));
            }

            int ans = 0;
            for (int cd : jack) if (jill.contains(cd)) ans++;
            pw.println(ans);
