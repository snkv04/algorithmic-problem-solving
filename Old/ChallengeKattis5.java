...
public class ChallengeKattis5 {
    ...
    static void solve() throws IOException {
        ...
        pw.println(Arrays.toString(place));
        
        SegmentTree st = new SegmentTree(new int[n+1]);
        // int last = place[1] - 1;
        int last = 0;
        Stack<Pair> gaps = new Stack<>();
        int i = 1;
        while (!q.isEmpty()) {
            int wantToPlace = q.poll();
            if (wantToPlace > last) {
                if (wantToPlace > last+1) {
                    int gap = wantToPlace - last - 1;
                    gaps.add(new Pair(gap, i));
                }
                last = wantToPlace;
            } else {
                // wantToPlace == last
                if (gaps.isEmpty()) {
                    st.update(i, st.query(i, i) + wantToPlace-i);
                    last = wantToPlace;
                } else {
                    //
                }
