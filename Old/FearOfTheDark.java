...
public class FearOfTheDark {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            px = Integer.parseInt(line[0]); py = Integer.parseInt(line[1]);
            line = br.readLine().split(" ");
            ax = Integer.parseInt(line[0]); ay = Integer.parseInt(line[1]);
            line = br.readLine().split(" ");
            bx = Integer.parseInt(line[0]); by = Integer.parseInt(line[1]);
    ...
    static boolean check(double mid) {
        if ((dist(ax, ay, 0, 0) <= mid && dist(ax, ay, px, py) <= mid)
            || (dist(bx, by, 0, 0) <= mid && dist(bx, by, px, py) <= mid)
            || ((dist(ax, ay, 0, 0) <= mid && dist(bx, by, px, py) <= mid || dist(ax, ay, px, py) <= mid && dist(bx, by, 0, 0) <= mid) && (2 * mid >= dist(ax, ay, bx, by)))) {
