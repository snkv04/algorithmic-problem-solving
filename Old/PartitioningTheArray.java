...
public class PartitioningTheArray {
    ...
    static void solve() throws IOException {
        ...
        for (int f : factors) {
            ...
            // for (int i = 0; i < f; i++) {
            //     int gcd = Math.abs(a[i+f]-a[i]);
            //     for (int j = i; j <= n-1-f; j += f) {
            //         gcd = (int) gcd(gcd, Math.abs(a[j+f]-a[j]));
            //     }
            //     gcds.add(gcd);
            // }
            ...
            //     gcd  = (int) gcd(gcd, gcds.get(i));
            // }
            // if (gcd != 1) {
            //     ans++;
            // }
            int gcd = Math.abs(a[f]-a[0]);
            for (int i = 0; i <= n-f-1; i++) {
                gcd = (int) gcd(gcd, Math.abs(a[i+f]-a[i]));
            }
            if (gcd != 1) ans++;
        }
        pw.println(ans);
