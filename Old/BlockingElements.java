...
public class BlockingElements {
    ...
    static void solve() throws IOException {
        ...
        memo = new HashMap<>();
        worst = -1;
        pw.println(best(0, 0, 0, 0));
    }

    static long best(long maxSum, long currSum, long dividers, int index) {
        if (index == n) {
            return Math.max(Math.max(maxSum, currSum), dividers);
        }
        String identifier = maxSum+" "+currSum+" "+dividers+" "+index;
        if (memo.containsKey(identifier)) return memo.get(identifier);

        long val = Long.MAX_VALUE / 4;

        // don't place a divider on it
        currSum += a[index];
        val = Math.min(val, best(Math.max(maxSum, currSum), currSum, dividers, index+1));

        // place a divider on it
        currSum = 0;
        val = Math.min(val, best(maxSum, 0, dividers + a[index], index+1));

        memo.put(identifier, val);
        return val;
    }
