...
public class ClassScheduling {
    ...
    static HashMap<String, ArrayList<Interval>> times;
    ...
    static void solve() throws IOException {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            String name = line[0];
            int choices = Integer.parseInt(line[1]);
            times.put(name, new ArrayList<>());
            while (choices-->0) {
                line = br.readLine().split(" ");
                times.get(name).add(new Interval(timeToNum(line[0]), timeToNum(line[1])));
            }
        }
        // pw.println(times);
        ...
        int index = 0;
        for (Map.Entry<String, ArrayList<Interval>> entry : times.entrySet()) {
            classNames[index] = entry.getKey();
            index++;
        }
        makeSchedule(0, new TreeSet<Interval>(Comparator.comparingInt(o -> o.l)));
        pw.println(ans);
    }

    static void makeSchedule(int classNum, TreeSet<Interval> currSched) {
        for (Interval classTime : times.get(classNames[classNum])) {
            boolean intersect = (currSched.floor(classTime) != null && currSched.floor(classTime).r >= classTime.l)
                || (currSched.ceiling(classTime) != null &&  currSched.ceiling(classTime).l <= classTime.r);
            boolean suboptimal = !currSched.isEmpty()
                && (currSched.last().r - classTime.l > ans || classTime.r - currSched.first().l > ans);
            if (intersect || suboptimal) continue;

            currSched.add(classTime);
            if (classNum == n-1) {
                // if (currSched.last().r-currSched.first().l < ans) pw.println(currSched);
                ans = Math.min(ans, currSched.last().r-currSched.first().l);
            } else {
                makeSchedule(classNum+1, currSched);
            }
            currSched.remove(classTime);
        }
    }

    static int timeToNum(String time) {
        String[] sections = time.split(":");
        return Integer.parseInt(sections[0])*60 + Integer.parseInt(sections[1]);
    }
