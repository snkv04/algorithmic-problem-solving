...
public class GhostLeg {
    ...
    static void solve() throws IOException {
        ...
        TreeSet<rung>[] rungs = new TreeSet[n+1];
        for (int i = 1; i <= n; i++) rungs[i] = new TreeSet<>(Comparator.comparingInt(o -> o.height));
        for (int i = 1; i <= m; i++) {
            int left = Integer.parseInt(br.readLine());
            rungs[left].add(new rung(i, left+1));
            rungs[left+1].add(new rung(i, left));
        }

        for (int i = 1; i <= n; i++) {
            int vertical = i, last = 0;
            while (rungs[vertical].higher(new rung(last, -1)) != null) {
                rung r = rungs[vertical].higher(new rung(last, -1));
                vertical = r.to;
                last = r.height;
            }
            ordering[vertical] = i;
        }
        for (int i = 1; i <= n; i++) pw.println(ordering[i]);
    }
    
    static class rung {
        int height, to;

        public rung(int height, int to) {
            this.height = height;
            this.to = to;
        }
    }
