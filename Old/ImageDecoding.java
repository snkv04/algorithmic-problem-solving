        ...
        while (true) {
            ...
            for (int i = 1; i <= n; i++) {
                ...
                for (int j = 1; j <= split.length - 1; j++) {
                    int run = Integer.parseInt(split[j]);
                    length += run;
                    for (int k = 1; k <= run; k++) {
                        sb.append(c);
                    }
            ...
            if (!sameLength) {
                pw.println("Error decoding image");
            }

            /*
             * initialize a correct lenght  variable
             * initialize boolean samelength
             * now go throught he lines. for each line:
             *      record the first character, initialize a currentlinelength;
             *      for each integer:
             *          print that number of the current char, add int to current line length, and then switch char
             *      print the buffer
             *      if the line is the first line, set correct length = to length
             *      else check if the length = correct lenght, if not, then set samelength = false
             * at the end of all lines, if didn't wokr, then print error
             */
