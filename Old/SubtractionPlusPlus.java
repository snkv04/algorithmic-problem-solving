...
public class SubtractionPlusPlus {
    ...
    static void solve() throws IOException {
        ...
        // boolean[][] memo = new boolean[5001][5001];
        // for (int i = 1; i <= n; i++) {
        ...
        // pw.println("duration = "+Duration.between(before, after).toMillis());

        // for (int i = 0; i <= 50; i++) pw.println("memo["+i+"] = " + memo[i][1]);

        // pw.println(memo[n][1]?"YES":"NO");

        int n = Integer.parseInt(br.readLine());
        boolean[] memo = new boolean[5001];
        int index = 0;
        for (int i = 1; index <= 5000; i++) {
            index += i;
            for (int j = 1; j <= i && index <= 5000; j++) {
                memo[index++] = true;
            }
        }
        // for (int i = 0; i <= 50; i++) System.out.println("memo["+i+"] = "+memo[i]);
        pw.println(memo[n]?"YES":"NO");
    }
