...
public class WordLadder2 {
    ...
    static int[] distFrom0, distFrom1;
    ...
    static void solve() throws IOException {
        ...
        // toNum = new HashMap<>(); toStr = new ArrayList<>(); counter = 0;
        // for (int i = 0; i < n; i++) {
        //     String word = br.readLine();
        //     if (!toNum.containsKey(word)) {
        //         toNum.put(word, counter++);
        //         toStr.add(word);
        //     }
        // }
        String[] words = new String[n]; for (int i = 0; i < n; i++) words[i] = br.readLine();
        ...
        distFrom0 = new int[n]; Arrays.fill(distFrom0, (int) 1e9); distFrom0[0] = 0;
        ...
        while (!q.isEmpty()) {
            ...
            for (int next : adj[node]) {
                if (!visited[next]) {
                    distFrom0[next] = distFrom0[node] + 1;
                    q.add(next);
                    visited[next] = true;
                }
            }
        }
        // pw.println(Arrays.toString(distFrom0));

        distFrom1 = new int[n]; Arrays.fill(distFrom1, (int) 1e9); distFrom1[1] = 0;
        visited = new boolean[n];
        q = new LinkedList();
        q.add(1);
        while (!q.isEmpty()) {
            ...
            for (int next : adj[node]) {
                if (!visited[next]) {
                    distFrom1[next] = distFrom1[node] + 1;
                    q.add(next);
                    visited[next] = true;
                }
            }
        }
        // pw.println(Arrays.toString(distFrom1));

        int original = distFrom0[1];
        int minDist = (int)1e9; String best = "";
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (i == j) continue;
                String s1 = words[i], s2 = words[j];
                if (dist(s1, s2) == 2) {
                    int newDist = distFrom0[i] + 2 + distFrom1[j];
                    if (newDist < minDist) {
                        String newWord = minConnector(s1, s2);
                        if (allWords.contains(newWord)) continue;
                        best = newWord;
                        minDist = newDist;
                    } else if (newDist == minDist) {
                        String newWord = minConnector(s1, s2);
                        if (allWords.contains(newWord)) continue;
                        if (newWord.compareTo(best) < 0) {
                            best = newWord;
                        }
                    }
                }
        ...
        if (minDist < original && minDist <= 1005) {
            pw.println(best);
            pw.println(minDist);
        } else {
            if (original >= (int)1e8) {
                pw.println(0); pw.println(-1);
            } else {
                pw.println(0); pw.println(original);
            }
        }
    }

    static String minConnector(String s1, String s2) {
        int diffs = 0, forSecond = -1;
