...
public class LongJumps {
    ...
    static void solve() throws IOException {
        ...
        HashSet<Integer> marks = new HashSet<>(); for (int num : a) marks.add(num);
        boolean xGood = false, yGood = false;
        for (int i = 1; i <= n-1; i++) {
            if (marks.contains(a[i]-x)) xGood = true;
            if (marks.contains(a[i]-y)) yGood = true;
        }
        if (xGood && yGood) pw.println(0);
        else if (xGood) {
            pw.println("1\n"+y);
        } else if (yGood) {
            pw.println("1\n"+x);
            ...
            for (int i = 0; i <= n-1 && !works; i++) {
                if (a[i]-x >= 0) {
                    int newMark = a[i]-x;
                    if (marks.contains(newMark+y)) {works = true;mark = newMark;}
                    if (marks.contains(newMark-y)) {works = true;mark = newMark;}
                }
                if (a[i]+x<=l) {
                    int newMark = a[i]+x;
