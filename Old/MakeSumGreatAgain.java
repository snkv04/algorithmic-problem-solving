...
public class MakeSumGreatAgain {
    static int MOD = (int) (1e9 + 7); // 998244353;
    static int n;
    static long s;
    static long[] a;
    static TreeSet<Long> nums;
    ...
    public static void main(String[] args) throws IOException {
        ...
        long sum = 0;
        nums = new TreeSet(); for (long num : a) { nums.add(num); sum += num;}
        
        long l = 0, r = Integer.MAX_VALUE, ans = -1;
        while (l <= r) {
            long mid = (l + r) / 2;
            if (check(mid, sum)) {
                ans = mid;
    ...
    static boolean check(long mid, long sum) {
        long last = 0, currSum = sum;
        for (long curr : nums) {
            if (last + mid >= curr) {
                currSum += ((curr-1) * (curr-1+1) / 2) - (last * (last+1) / 2);
                mid -= curr-last-1;
                last = curr;
            } else {
                currSum += ((last+mid) * (last+mid+1) / 2) - (last * (last+1) / 2);
                break;
            }
        }
        return currSum < s;
