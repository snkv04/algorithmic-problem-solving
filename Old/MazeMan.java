...
public class MazeMan {
    ...
    static void solve() throws IOException {
        ...
        n = Integer.parseInt(asdf[0]); m = Integer.parseInt(asdf[1]);
        grid = new char[n][m];
        for (int i = 0; i <= n-1; i++) {
        ...
        int ans = 0;
        for (int i = 0; i <= n-1; i++) {
            if (grid[i][0] >= 'A' && grid[i][0] <= 'W') ans += dfs(i, 1) ? 1 : 0;
            if (grid[i][m-1] >= 'A' && grid[i][m-1] <= 'W') ans += dfs(i, m-2) ? 1 : 0;
        }
        for (int j = 1; j <= m-2; j++) {
            if (grid[0][j] >= 'A' && grid[0][j] <= 'W') ans += dfs(1, j) ? 1 : 0;
            if (grid[n-1][j] >= 'A' && grid[n-1][j] <= 'W') ans += dfs(n-2, j) ? 1 : 0;
        }
        // for (int i =0 ; i <= n-1 ;i++) {for (int j =0 ; j <=m-1; j++) pw.print(visited[i][j]?'o':'_'); pw.println();}
        int ans2 = 0;
        for (int i = 0; i <= n-1; i++) {
            for (int j =0 ; j <= m-1; j++) {
                if ((!visited[i][j]) && grid[i][j] == '.') ans2++;
            }
        }
        pw.println(ans+" "+ans2);
    }
    ...
    static boolean dfs(int i, int j) {
        ...
        if ((int)grid[i][j] >= (int)'A' && (int)grid[i][j] <= (int)'X') return false;

        // pw.println("doing dfs on i="+i+", j="+j);
        boolean dot = grid[i][j] == '.';
        visited[i][j] = true;
        for (int k = 0; k <= 3; k++) {
            dot = dfs(i+di[k], j + dj[k]) || dot;
        }
        return dot;
    }
