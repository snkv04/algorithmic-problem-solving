...
public class OutstandingImpressionist {
    ...
    static void solve() throws IOException {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            int li = Integer.parseInt(s[0])-1, ri = Integer.parseInt(s[1])-1;
            l[i] = li; r[i] = ri;
            if (li == ri) {
                count[li]++;
            }
        ...
        for (int i = 0; i < n; ++i) {
            boolean good = false;
            if (l[i] == r[i]) {
                good = st.query(l[i], l[i]) == 1;
            } else {
                good = st.query(l[i], r[i]) == 0;
            }
            pw.print(good ? 1 : 0);
        }
        pw.println();
    ...
    static class SegmentTree {
        ...
        public void build(int[] a, int vertex, int vl, int vr) {
            if (vl == vr) {
                ...
                t[vertex] = Math.min(t[2*vertex], t[2*vertex+1]);
            }
        }
    ...
    static class Lazy {
        ...
        public long min(int l, int r, int v, int vl, int vr) {
            if (vr < l || vl > r) return Long.MAX_VALUE / 2;
            if (l <= vl && vr <= r) return t[v];
            
            pushDown(v, vl, vr);
            int vm = (vl + vr) / 2;
            return Math.min(min(l, r, 2*v, vl, vm), min(l, r, 2*v+1, vm+1, vr));
        }
