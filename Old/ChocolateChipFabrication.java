...
public class ChocolateChipFabrication {
    ...
    static void solve() throws IOException {
        ...
        for (int i = 0; i <= n-1; i++) grid[i] = br.readLine().toCharArray();
        ...
        for (int i = 0; i <= n-1; i++) {
            for (int j = 0; j <= m-1; j++) {
                ...
                int k = 0;
                if (i == n-1 || grid[i+1][j] != 'X') k++;
                if (i == 0 || grid[i-1][j] != 'X') k++;
                if (j == m-1 || grid[i][j+1] != 'X') k++;
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i <= 3; i++) {
                int I = p.i + di[i];
                int J = p.j + dj[i];
                if (I>=0 && I<=n-1 && J>=0 && J<=m-1 && grid[I][J]=='X' && time[I][J]==0) {
                    q.add(new Point(I, J));
                    time[I][J] = time[p.i][p.j] + 1;
                }
            }
        }
