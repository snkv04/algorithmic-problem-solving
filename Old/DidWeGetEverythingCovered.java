...
public class DidWeGetEverythingCovered {
    ...
    static int[][] memo;
    static int n, k, m; static char[] s;
    // static Queue<Integer> doesntWork;
    static Pair[][] next;
    static Stack<Integer> recurs, doesntwork;
    ...
    static void solve() throws IOException {
        ...
        m = Integer.parseInt(asdf[2]);
        s = br.readLine().toCharArray();
    
        ArrayList<Character> notInSequence = new ArrayList<>();
        HashSet<Character> seen = new HashSet();
        for (int i = 0; i <= m-1; i++) {
            seen.add(s[i]);
            if (seen.size() == k) {
                notInSequence.add(s[i]);
                seen.clear();;
            }
        }
        
        if (notInSequence.size() >= n) {
            pw.println("YES");
        } else {
            pw.println("NO");
            for (char c : notInSequence) pw.print(c);

            TreeSet<Character> missingAtEnd = new TreeSet<>();
            for (char c = 'a'; c <= (char) ('a'+k-1); c++) missingAtEnd.add(c);
            for (char c : seen) missingAtEnd.remove(c);
            for (int i = notInSequence.size()+1; i <= n; i++) pw.print(missingAtEnd.first());
