...
public class OlyaAndGameWithArrays {
    public static void main(String[] args) throws IOException {
        ...
        while (tc-->0) {
            ...
            // basically checking to see which array we can add the global minimum to
            // (as well as all the other minima) to reduce the total sum by the least amount
            for (int i = 0; i <= n-1; i++) {
                if (lists[i].get(1) < minOther) {
                    minOther = lists[i].get(1);
                }
            }
            sum -= minOther - min;
            pw.println(sum);
