...
public class GearsAndAxles {
    ...
    static void solve() throws IOException {
        ...
        while (n-->0) {
            ...
            int size = a[0], count = a[1];
            if (!map.containsKey(size)) {
                map.put(size, new ArrayList<Integer>());
            }
            map.get(size).add(count);
        }
        // pw.println(map);
        ...
        for (Map.Entry<Integer, ArrayList<Integer>> entry : map.entrySet()) {
            ...
            // double multiplier = (double) entry.getValue().last() / (double) entry.getValue().first();
            // ans += Math.log(multiplier);
            // pw.println("between "+entry.getValue().last()+" and "+entry.getValue().first()+" for tooth size "+entry.getKey()+", m="+multiplier);
            // pw.println("\tso the log is "+Math.log(multiplier));

            // for (int i = 0; i <= list.size() - 2; i++) {
            //     double multiplier = (double) list.get(i+1) / (double) list.get(i);
            //     ans += Math.log(multiplier);
            //     pw.println("between "+list.get(i+1)+" and "+list.get(i)+" for tooth size "+entry.getKey()+", m="+multiplier);
            //     pw.println("\tso the log is "+Math.log(multiplier));
            // }
            
            ArrayList<Integer> sorted = new ArrayList<>(entry.getValue());
            Collections.sort(sorted);
            // while (sorted.size() >= 2) {
            //     double multiplier = (double) sorted.pollLast() / (double) sorted.pollFirst();
            //     ans += Math.log(multiplier);
            // }
            for (int i = 0; i < sorted.size()/2; i++) {
                double multiplier = (double) sorted.get(sorted.size()-1-i) / (double) sorted.get(i);
                ans += Math.log(multiplier);
            }
        }
        pw.println(ans);
