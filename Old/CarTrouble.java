...
public class CarTrouble {
    ...
    static void solve() throws IOException {
        /*
         * read in edges and construct G and G^{T}, mapping nodes to indices and vice versa
         * also while reading in input, maintain order of reading in
         * run dfs on G and have one visited array for unreachable
         * run dfs on the transpose of G and have another visited array for trapped
         * iterate throguh original order once for trapped and then again for unreachable
         */

        int n = Integer.parseInt(br.readLine());
        ArrayList<Integer> inputOrder = new ArrayList<>();
        HashSet<Integer> used = new HashSet<>();
        ArrayList<Integer>[] adj = new ArrayList[1000], transpose = new ArrayList[1000];
        for (int i = 0; i <= 999; i++) { adj[i] = new ArrayList<>(); transpose[i] = new ArrayList<>(); }
        for (int i = 0; i <= n-1; i++) {
            ...
            for (int j = 2; j <= split.length-1; j++) {
                int adjacent = Integer.parseInt(split[j]);
                // if (!used.contains(adjacent)) {
                //     used.add(adjacent);
                //     inputOrder.add(adjacent);
                // }
                adj[node].add(adjacent);
                transpose[adjacent].add(node);
            }
        }
        // for (int i = 0; i <= n-1; i++) {
        //     pw.println("adj["+i+"] = "+adj[i]);
        // }
        // for (int i = 0; i <= n-1; i++) {
        //     pw.println("transpose["+i+"] = "+transpose[i]);
        // }
        // pw.println("inputOrder = "+inputOrder);
        ...
        boolean[] canReach0 = new boolean[1000];
        q = new LinkedList<>();
        q.add(0);
        while (!q.isEmpty()) {
            int node = q.poll();
            if (canReach0[node]) continue;
        ...
        for (int i = 0; i <= n-1; i++) {
            int node = inputOrder.get(i);
            if (!canReach0[node]) {
                pw.println("TRAPPED "+node);
                problemExists = true;
            }
        }
        for (int i = 0; i <= n-1; i++) {
            int node = inputOrder.get(i);
            if (!reachable[node]) {
                pw.println("UNREACHABLE "+node);
                problemExists = true;
            }
        }
        if (!problemExists) {
            pw.println("NO PROBLEMS");
        }
    }
