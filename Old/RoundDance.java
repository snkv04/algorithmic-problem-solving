...
public class RoundDance {
    ...
    static int count, comps; static boolean deg1;
    ...
    static void solve() throws IOException {
        ...
        adj = new HashSet[n]; for (int i =0; i <= n-1; i++) adj[i] = new HashSet<>();
        for (int i = 0; i <= n-1; i++) {
            adj[i].add(a[i]);
            adj[a[i]].add(i);
        }
        ...
        for (int i = 0; i <= n-1; i++) {
            deg1 = false;
            if (!visited[i]) {comps++;dfs(i);if(deg1)count++;}
        }
        count--;
        count = Math.max(count, 0);
        pw.println((comps-count)+" "+comps);
    }

    static void dfs(int node) {
        visited[node] = true;
        if (adj[node].size()==1) deg1 = true;
        for (int next : adj[node]) if (!visited[next]) dfs(next);
    }
