...
public class VikaAndPriceTags {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            TreeSet<Integer> zero = new TreeSet<>(), one = new TreeSet<>(), two = new TreeSet<>();
            for (int i = 0; i <= n-1; i++) {
                if (a[i]==0 && b[i]==0) {
                    zero.add(i); one.add(i); two.add(i);
                } else if (a[i] == b[i]) {
                    zero.add(i);
                } else {
                    int timeSteps = 2;
                    int secondLast = a[i], last = b[i];
                    if (secondLast < last) {
                    ...
                    while (last != 0) {
                        int q = secondLast / last, r = secondLast % last;
                        // q >= 1
                        int elementsAdded = ((q-1)/2) * 3 + ((q-1)%2 + 1);
                        if (q % 2 == 0) {
                            secondLast = r + last;
                            last = r;
                        } else {
                        ...
                        timeSteps += elementsAdded;
                    }
                    if (timeSteps%3==0) zero.add(i);
                    else if (timeSteps%3==1) one.add(i);
                    else two.add(i);
                }
            }

            boolean works = zero.size()==n || one.size()==n || two.size()==n;
