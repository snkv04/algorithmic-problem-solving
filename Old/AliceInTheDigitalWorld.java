...
public class AliceInTheDigitalWorld {
    ...
    static void solve() throws IOException {
        ...
        boolean[] greater = new boolean[n+1];
        int[] prefsums = new int[n+1];
        for (int i = 1; i <= n; i++) {
            prefsums[i] = prefsums[i-1] + a[i-1];
            if (a[i-1] == m) indices.add(i);
            if (a[i-1] > m) greater[i] = true;
        }
        ...
        for (int index : indices) {
            int l = index, r = index;
            while (true) {
                if (l == 1 || !greater[l-1]) break;
                else l--;
            }
            while (true) {
                if (r == n || !greater[r+1]) break;
                else r++;
            }
            ans = Math.max(ans, prefsums[r] - prefsums[l-1]);
        }
        pw.println(ans);
    }
