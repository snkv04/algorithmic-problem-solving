...
public class MEXRepetition {
    public static void main(String[] args) throws IOException {
        ...
        while (t-->0) {
            ...
            TreeSet<Integer> set = new TreeSet<>(); for (int i = 0; i <= n; i++) set.add(i);
            for (int num : a) { order.add(num); set.remove(num); }
            order.add(set.pollFirst());

            k %= n+1;
            int start = (n+1-k) % (n+1);
            for (int i = 0; i <= n-1; i++) {
                pw.print(order.get((start+i)%(n+1)) + " ");
            }
            pw.println();
        }
