...
public class PolycarpusDice {
    ...
    static void solve() throws IOException {
        ...
        long shown = Long.parseLong(asdf[1]);
        long[] d = Arrays.stream(br.readLine().split(" ")).mapToLong(Integer::parseInt).toArray();
        long sum = 0; for (long di : d) sum += di;
        for (int i = 0; i <= n-1; i++) {
            long others = n-1;
            long max = Math.min(shown-others, d[i]);
            others = sum - d[i];
            long min = Math.max(shown-others, 1);
            long available = Math.max(0, max-min+1);
            pw.print(d[i]-available+" ");
        }
    }
