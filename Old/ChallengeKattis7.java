...
public class ChallengeKattis7 {
    ...
    static void solve() throws IOException {
        ...
        int n = Integer.parseInt(inp[0]), k = Integer.parseInt(inp[1]), c = Integer.parseInt(inp[2]);
        nameToNum = new HashMap<>();
        numToName = new String[n];
        for (int i = 0; i <= n-1; i++) {
            numToName[i] = br.readLine();
        ...
        while (k-->0) {
            ...
            int a = nameToNum.get(inp[0]);
            int b = nameToNum.get(inp[1]);
            adj[a].add(b);
            adj[b].add(a);
            deg[a]++;
            deg[b]++;
        }
        int maxPerson = 0; for (int i = 1; i <= n-1; i++) if (deg[i] > deg[maxPerson]) i = maxPerson;
    ...
    static boolean color(int node, int[] color, int numColored) {
        boolean works = true;
