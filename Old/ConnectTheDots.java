...
public class ConnectTheDots {
    ...
    static void solve() throws IOException {
        ...
        int[][] endpoints = new int[n][11];

        while (m-->0) {
            str = br.readLine().split(" ");
            int a = Integer.parseInt(str[0])-1, d = Integer.parseInt(str[1]), k = Integer.parseInt(str[2]);
            int start = a, end = a + (d * k);
            endpoints[start][d]++;
            endpoints[end][d]--;
        }
        ...
        for (int step = 1; step <= 10; step++) {
            for (int mod = 0; mod < step; mod++) {
                int curr = 0;
                for (int idx = mod; idx < n; idx += step) {
                    curr += endpoints[idx][step];
                    if (curr > 0) {
                        adj[idx].add(idx + step);
                        adj[idx + step].add(idx);
                    }
                }
            }
        }
