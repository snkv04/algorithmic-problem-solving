...
public class WordPuzzle {
    ...
    static void solve() throws IOException {
        ...
        for (String shift : shifts) {
            // pw.println(shift);
            countSubsequences(shift.toCharArray(), p);
        }
        pw.println(ans);
    }

    static void countSubsequences(char[] regex, char[] full) {
        int[] memo = new int[regex.length+1];
        memo[0] = 1;
        for (int i = 0; i <= full.length-1; i++) {
            for (int j = regex.length; j >= 1; j--) {
                memo[j] += full[i] == regex[j-1] ? memo[j-1] : 0;
                memo[j] %= MOD;
            }
        }
        ans += memo[regex.length];
        ans %= MOD;
    }
