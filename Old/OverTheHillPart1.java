...
public class OverTheHillPart1 {
    ...
    static void solve() throws IOException {
        ...
        String _map = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 ";
        for (int i= 0; i <= 36; i++) map.put(_map.charAt(i), i);
        ...
        while (true) {
            ...
            for (int j = 0; j < n; j++) {
                substring[j] = (i-n+j<s.length())?map.get(s.charAt(i-n+j)):36;
            }
            for (int row = 0; row <= n-1; row++) {
                int ans = 0;
                for (int c = 0; c < n; c++) {
                    ans += matrix[row][c] * substring[c];
                }
                result.add(_map.charAt(ans%37));
            }

            if (i >= s.length()) break;
        }
        for (int j = 0; j < result.size(); j++) pw.print(result.get(j));
        pw.println();
    }
