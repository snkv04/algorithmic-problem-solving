...
public class CardGame {
    ...
    static void solve() throws IOException {
        ...
        String suits = "CDHS";
        for (int i = 0; i <= 3; i++) { map.put(suits.charAt(i), i); }

        int n = Integer.parseInt(br.readLine());
        int trump = map.get(br.readLine().charAt(0));
        PriorityQueue<Integer>[] cards = new PriorityQueue[4]; for (int i = 0; i <= 3; i++) cards[i] = new PriorityQueue<>();
        String[] asdf = br.readLine().split(" ");
        for (String card : asdf) {
            int suit = map.get(card.charAt(1));
            cards[suit].add(card.charAt(0)-'0');
        }
        // for (int i = 0; i <= 3; i++) pw.println("cards["+i+"] = "+cards[i]);
        ...
        for (int i = 0; i <= 3; i++) {
            ...
            while (cards[i].size() >= 2) {
                ans.add(cards[i].poll()+""+suits.charAt(i));
                ans.add(cards[i].poll()+""+suits.charAt(i));
            }
            if (cards[i].size() == 1) {
                if (cards[trump].isEmpty()) {
                    pw.println("IMPOSSIBLE");
                    return;
                }
                ans.add(cards[i].poll()+""+suits.charAt(i));
                ans.add(cards[trump].poll()+""+suits.charAt(trump));
            }
        }
        while (!cards[trump].isEmpty()) {
            ans.add(cards[trump].poll()+""+suits.charAt(trump));
            ans.add(cards[trump].poll()+""+suits.charAt(trump));
        }

        for (int i = 0; i <= n-1; i++) {
            pw.println(ans.get(2*i)+" "+ans.get(2*i+1));
        }
    }
    ...
    static class SegmentTree {
        ...
        public void build(int[] a, int vertex, int vl, int vr) {
            if (vl == vr) {
                ...
                // System.out.println("leftchild = "+t[2*vertex]+", rightchild = "+t[2*vertex+1]);
                // System.out.println("m="+m);
                t[vertex] = (t[2*vertex] * t[2*vertex+1]) % m; // computes the higher-level node afterward
            }
        }
        ...
        public void update(int ind, int newVal, int vertex, int vl, int vr) {
            if (vl == vr) {
                ...
                t[vertex] = (t[2*vertex] * t[2*vertex+1]) % m;
            }
        }
