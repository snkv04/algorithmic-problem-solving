...
public class XORDistance {
    ...
    static void solve() throws IOException {
        ...
        long a = Long.parseLong(asdf[0]), b = Long.parseLong(asdf[1]), r = Long.parseLong(asdf[2]);
        PriorityQueue<Integer> diffs = new PriorityQueue<>(Comparator.comparingInt(o -> -o)); // sorts by largest first
        for (int i = 0; i <= 60; i++) {
            long abit = (((long)1) << i) & a;
            long bbit = (((long)1) << i) & b;
            if (abit != bbit) {
                diffs.add(i);

                // just ensures the num with the largest diff is a
                if (abit < bbit) {
                    long temp = a;
                    a = b;
                    b = temp;
        ...
        // pw.println("a = " + Long.toBinaryString(a));
        // pw.println("b = " + Long.toBinaryString(b));
        // pw.println("diffs = "+diffs);

        long biggest = ((long) 1) << diffs.poll();
        long sum = 0;
        while (!diffs.isEmpty()) {
            int nextPos = diffs.poll();
            long abit = (((long)1) << nextPos) & a;
            long bbit = (((long)1) << nextPos) & b;

            if (bbit > 0) {
                // a's bit is 0, b's bit is 1
                sum += bbit;
                // pw.println("setting bit "+nextPos+" of x to 0");
            } else {
                // a's bit is 1, b's bit is 0
                if ((((long)1)<<nextPos) <= r) {
                    sum += (((long)1)<<nextPos);
                    r -= (((long)1)<<nextPos);
                    // pw.println("setting bit "+nextPos+" of x to 1");
                } else {
                    sum -= (((long)1)<<nextPos);
                }
            }
        }
        // pw.println("sum = "+sum+", biggest = "+biggest);
        // pw.println(biggest-sum+"\n");
        pw.println(biggest-sum);
    }
