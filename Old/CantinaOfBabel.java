...
public class CantinaOfBabel {
    ...
    static void solve() throws IOException {
        ...
        String[] speak = new String[n];
        HashSet<String>[] understand = new HashSet[n];
        for (int i= 0; i <= n-1; i++) understand[i] = new HashSet<>();
        for (int i = 0; i <= n-1; i++) {
            String[] asdf = br.readLine().split(" ");
            speak[i] = asdf[1];
            for (int j = 1; j <= asdf.length-1; j++) {
                understand[i].add(asdf[j]);
            }
        }
        ...
        for (int i= 0; i <= n-1; i++) {
            for (int j = 0; j <= n-1; j++) {
                // if (i == j) continue;
                if (understand[j].contains(speak[i])) adj[i].add(j);
            }
        }
